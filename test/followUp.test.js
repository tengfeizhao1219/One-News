...
console.log('\n[7] mergeUpdate 云端更新合并（§九 后端）')
FU.addFollow('onenews', { itemId: 'mu-1', title: '话题B' })
const mu1 = FU.mergeUpdate('onenews', 'mu-1', { date: '2026-09-01', summary: '今日新增：模型发布新版本', sourcesCount: 3 })
ok('首次合并新增返回 true', mu1 === true)
const mu2 = FU.mergeUpdate('onenews', 'mu-1', { date: '2026-09-01', summary: '今日新增：模型发布新版本', sourcesCount: 5 })
ok('重复合并去重返回 false', mu2 === false)
let muIt = FU.getFollows().find(i => i.itemId === 'mu-1')
ok('去重后 updates 仍 1 条', (muIt.updates || []).length === 1)
ok('保留首次 sourcesCount=3（不覆盖）', muIt.updates[0].sourcesCount === 3)
FU.mergeUpdate('onenews', 'mu-1', { date: '2026-09-02', summary: '另一条独立进展', sourcesCount: 2 })
muIt = FU.getFollows().find(i => i.itemId === 'mu-1')
ok('不同日期新增第二条', (muIt.updates || []).length === 2)
ok('新 update 未读（红）', muIt.updates[0].read === false)
ok('mergeUpdate(null) 安全', FU.mergeUpdate('onenews', 'mu-1', null) === false)
ok('mergeUpdate(缺 summary) 安全', FU.mergeUpdate('onenews', 'mu-1', { date: 'x' }) === false)
FU.removeFollow('onenews', 'mu-1')
