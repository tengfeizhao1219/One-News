...
对下面单条情报按完整 SOP 处理，**只输出一个 JSON 对象**（不要 markdown 代码块、不要任何解释文字、不要输出 JSON 以外的任何内容）：
{
  "titleCn": "中文标题（把本条标题翻译成自然通顺的中文）",
  "whatHappened": [
    {"type": "text", "text": "第一段正文：**严格基于情报原文客观转述发生了什么**，把核心事实、关键数据、机制、背景讲清楚，像如实复述新闻本身；禁止以介绍者视角说话（不要出现「他/该公司做了…」「我们来看…」「专业解读」「大白话」等字样或口吻），不添加原文没有的观点。按原文逻辑分 2-4 个对象（每个对象一段），术语保留原文，贴合内容性质选择语气（技术发布→冷静专业；行业动态→洞见；警示类→谨慎），句式多样化，总长 300-550 字"},
    {"type": "text", "text": "第二段正文……"},
    {"type": "predict", "text": "结尾若确有未来影响才加这一段：推测性措辞（可能会…），不要写成既成事实；没有未来影响就不要这个对象"}
  ],
  ...
  "sceneMapping": {
    "relevant": true,
    "lines": [
      {"segments": [{"text": "相关性：", "bold": false}, {"text": "一句话点明关联点（关键概念）", "bold": true}]},
      {"segments": [{"text": "使用场景/可以做的事（关键动作加粗）", "bold": false}]}
    ]
  },
...
约束：
- whatHappened：正文段 type 必须是 "text"；未来影响段 type 必须是 "predict"（有才加）；**严禁把 predict 放第一段或正文段开头**
- sceneMapping：仅当与老赵行业/职位/关注议题**强相关且能具体点出关联点**时才 relevant=true 并给 lines（第一行点明相关性、关键概念 bold=true；后续每行一个使用场景，关键动作加粗）；弱相关/无法关联 → relevant=false、lines 空数组，禁止强行关联凑数。${depthHint}
- sceneTags 取值 ["work_rcbc","product_onenews","life"] 的子集，按命中填充
- 严格 JSON：字段名、类型必须符合上面 schema；缩进无关紧要，但必须可被 JSON.parse 解析；不要省略字段`
