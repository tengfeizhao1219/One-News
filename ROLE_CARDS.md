...
> **用法**：为 5 个角色各新建一个会话，把下面**对应角色**的那一段整段复制进该会话即可。
...
| 全栈开发 | FS | 前端开发 + 后端开发 + 技术负责人 | 全栈权限 |
| 项目经理 | PJM | 项目经理（不变） | 无变化 |
| **Git 管理专家** | **GM** | **新增** | **统一所有 git 操作，解决多人并行提交冲突** |

> **GM 引入理由**：4 角色共享同一工作区，各自 git commit/push 导致频繁 merge conflict（320 次 commit 中 20 个 merge commit）。GM 统一管理 git，各角色只产出文件 + 写 staging 标记。
