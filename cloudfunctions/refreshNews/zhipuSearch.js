...
2. 每条新闻输出为 JSON 对象，包含以下字段：
   ...
   - content: AI 解读（字符串，100-600字，分两段撰写：第一段先把新闻本身讲清楚——发生了什么、核心事实与背景；第二段起为独立解读，形式自由，根据新闻内容自选最合适的角度深入，如深层原因剖析、对相关领域或普通人的影响、专业视角评判、历史背景或同类事件对比、未来走向判断等，选最有价值的1-2个角度即可，不必面面俱到。解读长度与新闻信息量匹配：信息量大可写长，信息量小则写短。这不是原文复述——综合多个信源，用独立客观的分析语气撰写。分段用\\n分隔）
...
const CATEGORY_PROMPTS = {
  ...
  science: buildPrompt('科学探索新闻', 'xinhuanet.com, cctv.com, thepaper.cn, reuters.com, nationalgeographic.com, science.org'),
