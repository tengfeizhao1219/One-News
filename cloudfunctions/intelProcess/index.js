...
务实、直接给结论，不堆参数、不软文、不夸大、不含糊其辞。英文专有名词保留原文（首现可括号注中文），不意译夸张措辞。${translatePref}`
  ...
  if (level === 'medium') {
    return {
      system: baseSystem + `\n对下面的单条情报做「轻量摘要」：
先输出「发生了什么」（**段落间用空行分隔**，总 180-320 字）：**严格基于情报原文客观转述发生了什么**——把原文的核心事实、关键数字、机制讲清楚，**像如实复述新闻本身**；**禁止出现「他/该公司/我们」「专业解读」「大白话」等介绍性口吻或标签字样**，不要以"我/我给你讲"的视角说话；结尾若确有未来影响可加一段，开头仅用「（AI 预测）」作段落标记给一句推测（该标记仅供程序识别，不要展开成正文文字）。**语言风格贴合内容**（技术→冷静专业/社区→轻松），句式多样化，避免八股腔。**禁止只写一句话、禁止写成一整段不换行。**
...
0. 发生了什么（最关键，结构要求见下）：**严格基于情报原文客观转述发生了什么**，把核心事实、关键数字、机制、背景讲清楚，像如实复述新闻本身；**禁止以介绍者视角说话**——不要出现「他/该公司做了…」「我们来看…」「专业解读」「大白话」等字样或口吻，不添加原文没有的观点。**段落之间必须用空行分隔**，总长 300-550 字：
   ...
   - 结尾若确有未来影响：可加一段，开头仅用「（AI 预测）」作段落标记给一句推测（该标记仅供程序识别，不要展开成正文文字），用推测性措辞（"可能会…"），不要写成既成事实。
