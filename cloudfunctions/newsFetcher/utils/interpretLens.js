...
=======
>>>>>>> Stashed changes
  // 2026-08-19 版权红线：讲新闻本身的事实段同样不能照抄，连续引用原文不得超过 15 个字。
  // 事实必须用自己的话重新组织（改词、换序、合并背景），而不是把原文句子搬进来。
  lines.push(
    `${n++}. 版权条款：写事实段是用自己的话把这件事讲清楚，不是抄原文。任何时候都不能连续照搬原文超过 15 个字` +
    '——只准用自己的表述转述事件、数字、背景，即便讲「发生了什么」也不准整句引用原文；需要强调的个别名词可以出现，但不能连着抄一整句或半句。'
  )
<<<<<<< Updated upstream
=======
>>>>>>> main
