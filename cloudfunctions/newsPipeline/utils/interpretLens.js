...
function buildPrompt(route, opts = {}) {
  ...
  lines.push(`${n++}. 分两段写：第一段先把新闻本身讲清楚（发生了什么、核心事实与背景）；第二段起为独立解读，形式自由，根据内容自选 1-2 个最有价值的角度深入（如深层原因、对谁的影响、专业视角、历史背景/同类对比、未来走向等），不必面面俱到。`)
  ...
  lines.push(
    ...
    '禁止编造原文中不存在的事实；禁止用「总而言之」「关键有两点」「接下来最重要的是」等固定句式收尾。'
  )
  lines.push(
