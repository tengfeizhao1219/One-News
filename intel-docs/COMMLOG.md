...
| 日期 | 角色 | 事项 | 状态 |
|---|---|---|---|
| 2026-09-08 | Auto | **getNewsDetail 云函数缺 wx-server-sdk 已修复 + 详情页冷启动兜底**。真机「前往小程序」进详情页显示「网络开了个小差」。根因：getNewsDetail 云端部署缺 wx-server-sdk 依赖（`Cannot find module 'wx-server-sdk'`，部署事故，与 intelSearch 同类）→ 详情页冷启动直载一调必崩；正常首页进详情走首页透传数据不触发。修复：①恢复提交版 v6.2+装 wx-server-sdk 重新部署（6.7MB 含依赖，实测 getNewsDetail(newsId) code=0 返回正文）②前端 detail.js _loadFallback 失败路径回退渲染打包摘要卡（_cardFallback）免死错误页（新新闻才走真实详情；旧闻滚出缓存时也兜底）。**注意**：另一 AI 会话正在重构 getNewsDetail(v6.4 WIP)，已将其磁盘改动备份/恢复未提交；其部署 v6.4 时必须带依赖（installDependency），否则重蹈覆辙。提交 cc23f2e | ✅ 已修复+推送预览 |
| 2026-09-08 | Auto | **朋友圈单页「前往小程序」修复：按 scene 区分单页/正常模式**。真机反馈：单页点「前往小程序」后仍显示单页卡片。根因：scene 1155（单页点前往小程序，正常模式打开）微信复用同一分享 query，home/detail onLoad 只认 options.card 未区分场景。修复：new utils/shareCard.js isSinglePageScene()（getEnterOptionsSync 优先）——1154 渲染单页卡片（现状保留）；1155/其他正常入口 home 落回 loadNews 真实首页、detail 用卡片内 news id 走 _initEngine→_loadFallback 单篇直载（完整内容+全交互），打包参数损坏无 id 时 reLaunch 回首页。提交 ab6b102，已 auto_preview。**待真机复验**：单页→前往小程序=真实首页/详情页 | ✅ 已推送预览 |
...
| 2026-09-01 | Auto | **关注后续后端（§九）落地**（owner 确认 4 决策：全量增量同步/站内红点/Tavily 复用/每天每话题最多 1 次）。新增：① `follow_up` 集合（openid+module+itemId 幂等 upsert、软删除保留历史）；② `syncFollowUp` 云函数（关注/取消/进入关注页增量同步 + get 拉取 updates）；③ `followUpCheck` 云函数（定时按 trackTime 检索：Tavily 主 + 智谱兜底 + DeepSeek 判新/摘要，lastCheckedDate 防同天重复，无进展不打扰，预算保护 30 话题/90s）；④ 4 定时器 08/12/18/21 北京（7 段 cron `0 0 8 * * * *`，与线上 newsFetcher 格式一致）；⑤ 前端接线：`utils/followUpSync.js`（云端同步桥）+ `utils/followUp.js mergeUpdate`（去重合并保留已读）+ detail×2 关注/取消同步 + followup-card 进入拉取合并。**实测**：followUpCheck force 触发「DeepSeek 最新进展」→ Tavily 4 来源 → DeepSeek 摘要（寒武纪适配 V3.2-Exp/V3.1-Terminus）→ 写入 updates ✅。单测 33/33 过。**待办**：T9.5 真机端到端验证（OPENID 上下文无法 mcporter 模拟）；T9.6 可选增强（立即检索/订阅消息）。部署：syncFollowUp/followUpCheck 已上线，env 用线上 intelSearch 同款真实 key | ✅ 已部署 |
...
| 2026-08-28 | Auto | **每日自动同步 2026-08-28 21:44：提交 0 个文件；— 无推送（无改动）；—；✅ 工作区干净。详情见 .git-auto/auto.log** | ✅ 已同步 |
| 2026-08-28 | Auto | **每日自动同步 2026-08-28 21:41：提交 0 个文件；— 无推送（无改动）；—；✅ 工作区干净。详情见 .git-auto/auto.log** | ✅ 已同步 |
...
## 2026-08-22 · AI 摘要长度恢复 50-150 字（owner 拍板，硬截断保留）
