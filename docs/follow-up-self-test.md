...
- [ ] One News 首页**空白处按住不动 ≥0.5s** → **触发震动提示（无进度环倒计时动画）** → **覆盖层从按压点 clip-path 0%→150% 炸开**（无系统右滑转场，**绽放时长约 0.64s**）
- [ ] 情报官首页**同款长按** → 震动 + 覆盖层炸开进入
...
### 2.4 「我的关注」聚合页（followup-card 覆盖层组件）—— 四态与操作（UI 对齐 demo）
- [ ] 卡片为**圆角卡片**（border-radius 16rpx + 细描边 + 轻投影），非平铺分隔线
- [ ] 抬头：标题（左）+ **状态徽标**（有更新红「N 条新更新」/ 已读完绿 / 已是最新灰）
- [ ] 元信息：**来源标签**（资讯 蓝 / 情报官 紫）+「每天 X 追踪」+「已关注 N 天」
- [ ] 有更新/已读卡片：浅底高亮「最新更新框」展示最新一条；无更新：灰态「已检索，暂无新进展」
- [ ] 历史时间线：左侧竖线 + 圆点，展开后显示每日 AI 追踪条目（日期/摘要/来源数）；「展开历史更新 ▾ / 收起 ▴」切换
- [ ] 顶部汇总行：`N 个话题 · M 条新进展`
- [ ] 空状态：无关注时显示空态 + 「去首页看看」
- [ ] 点击卡片：**展开时间线**，且自动 `"标记已读"`（徽标红→绿）
