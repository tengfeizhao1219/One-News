...
## 10. UI 设计输入清单（产品设计师必读）

> **流程闸门（owner 2026-08-03 19:01）**：本章列出的所有前端展示项，必须由产品设计师完成**详细页面设计**，经 **PM + owner 评审通过**后，全栈开发方可开发对应页面/组件；测试验收在本章设计全部通过后进行。
> **设计规范约束**：遵循 `D-03-设计规范-v2增量修订.md` 与 `D-03-视觉设计-阶段二视觉稿.md`（字号/圆角/色彩 token/间距体系）；**暗色模式必须适配**（`theme.json` CSS 变量，不得硬编码颜色）；触控热区 ≥ 88rpx；无图/加载失败展示占位。

### 10.1 浏览记录页 `pages/history/history`（对应 TL-B14，依赖 UI-B4）

| # | 设计项 | 要求 |
|---|--------|------|
| UI-01 | 页面框架 | 导航栏标题「浏览记录」；内容区列表 + 空态 + 加载态三态齐备 |
| UI-02 | 列表项布局 | 标题（2 行截断，28rpx 主文字色）＋ 次行「来源 · 相对时间」（22rpx 次级文字色）；项间距/分隔线遵循 D-03 列表规范 |
| UI-03 | 相对时间格式 | 今天 → `HH:mm`；昨天 → `昨天`；2~7 天 → `N 天前`；超过 7 天不展示 |
| UI-04 | 空态 | 图标（时钟 🕘，64rpx）+ 文案「暂无浏览记录」+ 引导按钮「去首页看看」（跳首页） |
| UI-05 | 加载态 | 骨架屏 3~5 条占位（亮/暗色各一套） |
| UI-06 | 交互 | 点击列表项 → 跳详情页并定位该新闻；返回时回到记录页原位置 |
| UI-07 | 按压态 | 列表项按压反馈（浅色遮罩），热区 ≥ 88rpx |
| UI-08 | 暗色适配 | 亮/暗两套配色走查，重点对比度 ≥ 4.5:1 |

### 10.2 侧边栏「浏览记录」入口（对应 TL-B14）

| # | 设计项 | 要求 |
|---|--------|------|
| UI-09 | 入口条目 | 时钟图标 + 文案「浏览记录」，位于收藏列表**上方**，与收藏列表同级样式 |
| UI-10 | 徽标 | （可选）无新记录不显示徽标；不引入红点运营位 |

### 10.3 收藏云同步展示（对应 TL-B13 前端部分，依赖 UI-B5）

| # | 设计项 | 要求 |
|---|--------|------|
| UI-11 | 收藏列表打开 | 本地数据秒开 + 顶部细进度条（云端合并中，≤2s）；合并完成静默，无强提示 |
| UI-12 | 空态 | 保持现有空态（「暂无收藏」） |
| UI-13 | 待同步角标 | 离线收藏未上云条目：右下角小「☁️ 待同步」角标（12 号字，次级色）；点按重试 |
| UI-14 | 同步失败 | 整页顶部轻提示条「收藏同步失败，点击重试」（非阻断式） |

### 10.4 详情页反馈（对应 TL-B13 前端部分）

| # | 设计项 | 要求 |
|---|--------|------|
| UI-15 | 收藏反馈 | 收藏成功 ♥ 心跳动画（现状保留）；取消收藏恢复 ♡（现状保留）；**云端失败**时 toast「已收藏（待同步）」 |
| UI-16 | 分享反馈 | 点击分享调起转发卡片（现状 onShareAppMessage 不变）；无额外 UI |
| UI-17 | 新闻失效兜底 | 浏览记录回看已清理新闻 → 复用详情页 error 态（「新闻详情暂不可用，请返回重试」）或独立空态，由 UI 定稿 |

### 10.5 设计交付物与评审

1. 产品设计师输出：页面设计稿（含亮/暗两套）+ 标注稿（字号/间距/色值 token）；
2. 交付形式：`docs/02-产品设计/` 下新增 `D-06-下迭代三需求-UI设计.md`（沿用现有文档体系），可附 `showcase/` 可交互预览（参考 UX-B6 做法）；
3. **评审**：PM + owner 评审通过后，在 TASK_BOARD 标注「UI 设计 ✅」，TL-B13/TL-B14 前端部分方可开工。
...
## 12. 验收标准（开发自测 + PM 验收共用）

> **闸门说明**：§10 所列 UI 设计项（UI-01~UI-17）未全部通过评审前，TL-B13/TL-B14 前端部分不进入本章验收；后端/数据层不受限。
...
### 12.2 收藏/分享 30 天保留（RQ-03/RQ-07/RQ-16）
| # | 用例 | 期望 |
|---|------|------|
...
| F-07 | 侧边栏收藏列表打开 | 云端收藏合并：本地缺失条目由云端补入；**云端已删条目本地保留展示但不标「待同步」角标**（PM 口径 2026-08-05：与 F-06 离线收藏场景区分——离线未上云条目需保留并标角标，若按字面「云端已删移除」会导致离线收藏被误删；云端已删仅指已同步过、云端不再存在的条目，本地作为用户主动收藏资产保留展示） |

### 12.3 未收藏/未分享 7 天保留（RQ-16）
| # | 用例 | 期望 |
|---|------|------|
...
1. **设计**：产品设计师按 §10 输出 `D-06-下迭代三需求-UI设计.md`，PM + owner 评审通过（闸门）；
2. **开发**：全栈开发严格按本文档 §6~§9 实现（前端展示部分在设计通过后），提交时标注对应章节号；
3. **自测**：按 §12 自测清单执行，结果写入 `docs/05-测试验收/`；
4. **PM 验收**：开发完成后 PM 按 §12 逐条验收，输出验收报告；
5. **回归**：§12.4 回归用例 + 既有自动化测试全绿后方可合入。
