...
> **上游**：`PRD-RQ22-意见反馈留言板.md`（**v1.0 已确认**，PM）| **任务**：TASK_BOARD RQ-22-D（🔴 P0）
...
>
> **v1.0 变更（对齐 PRD v1.0 · PM commit `750853b` 12:39）**：
...
> - 字数限制 Q1 待确认（PRD 已确认「不限字数」，UI 层不硬性截断）
...
├── 管理计数条（仅作者）：留言总数「共 N 条」（Q4 待确认 · 不阻塞）
├── 留言列表（scroll-view · 倒序）
│   └── 留言卡片（card）
│       ├── 头：昵称（微信用户+随机 / 一页君+作者角标）+ 相对时间
│       ├── 内容：纯文本（多行 · 字数限制 Q1 待确认，已确认不限字数）
│       ├── 右下角（仅作者）：删除「🗑」按钮
│       └── 回复区（展开）
│           ├── 嵌套回复项（缩进 + 浅色底）：昵称（作者角标）+ 时间 + 内容
│           └── 回复输入框（点「回复」展开）
└── 底部固定输入区（留言板主输入）
    ├── 文本框（自适应高度 · 不限字数，Q1 待确认）
    └── 发送按钮（主题色 · disabled 态）
...
| 管理计数条（仅作者） | 计数 | `24rpx` · `--text-secondary` · padding `16rpx 32rpx 8rpx`（Q4 待确认） |
...
| 作者删除 | 留言卡**右下角**「🗑 删除」（仅 isAuthor 可见） | 确认弹窗 → feedback/delete 软删除（status='deleted'）→ 普通用户不可见、作者可查看（Q3 待确认） |
...
**待 owner 确认（不阻塞设计）**：Q1 字数限制（PRD 已确认不限字数，UI 不硬性截断）· Q3 软删除作者可见已删列表 · Q4 是否展示「共 N 条」。
