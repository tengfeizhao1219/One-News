# Git Staging 目录（⚠️ 已废弃）

> ⚠️ **已废弃（2026-08-05，v4.0）**：Git 管理专家（GM）角色已取消，全员直连 GitHub。各角色现在直接对自己职责范围内的文件执行 `git pull/add/commit/push`，**不再写入 staging 标记**，也不再等待 GM 统一提交。
> 本目录与其中的 `.ready` 文件仅作历史遗留，不再使用；`gm.ready` 已删除。新规范见 `ROLE_CARDS.md`（v4.0）与 `COLLABORATION.md` §七。
