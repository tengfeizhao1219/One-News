...
| UX-B6 | **全系统交互走查 + GitHub 展示页**（按最新设计语言审计 25 项交互，提 P0/P1/P2 修改建议；交付 `D-02-增量-全系统交互走查.md` + `showcase/index.html` 可交互预览） | ✅ | 无 | 已完成（走查覆盖 首页12/详情8/全局5 + RQ-15 待建；发现 `--primary` 悬空 / 「R」语义 / 顶栏热区 / §3 漂移 等；展示页镜像 theme.json Token，支持浅暗+字体档位+reduced-motion）<br>**补充 F12**：元信息 22rpx 不随字号缩放 → 按 D-02 §0「先标准后例外」登记入 §6.4 例外表，**与 PD-B7 红线同源**，交互侧只登记不推翻 |
