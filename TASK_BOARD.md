...
| 角色 | 编号 | 任务 | 优先级 | 依赖 | 变化 |
|------|------|------|:---:|------|------|
| **产品经理** | PD-B1 | ~~裁定 BUG-P1-011~~ → ✅ **owner 已裁定：前端下架农业/科学 tab**（2026-08-03 09:20 广播） | ✅ | 无 | 已闭环 |
...
| | FE-B6 | ~~若 PD-B1 裁定「下架 tab」→ 改 `utils/constants.js` 移除 agriculture/science~~ → ✅ 已由 PJM 代执行，闭环 | ✅ | — | 已闭环 |
| **全栈开发** | ~~BE-A1/A2~~ | ~~B-08 / B-11~~ | — | — | ❌ **撤回**（文件已不存在） |
| | BE-B1 | ~~若 PD-B1 裁定「后端补类」→ 在 `zhipuSearch.js` 的 `CATEGORY_PROMPTS` 增 agriculture/science 提示词~~ → ❌ 作废（裁定为前端下架） | ❌ | — | 已作废 |
