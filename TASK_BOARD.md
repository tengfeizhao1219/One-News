...
**一、owner 已确认 RQ-22 设计稿（13:11）**：`docs/02-产品设计/D-02-增量-RQ22-意见反馈留言板-UIUX设计.md`（v1.0）+ `demo/feedback-demo.html` 可交互 demo（已在 GitHub Pages 上线，交互原型库首页可直达）。

**二、设计要点（owner 确认后固化的最终口径）**：
- 留言板页 + 文明公约页，设置页「意见反馈」入口改 `navigateTo`
- 卡片式列表 + 底部固定输入区；多级嵌套回复（最深 4 级收平）
- **管理控件 isAuthor 驱动**：删除按钮（卡片右下角）+ 筛选（nav-bar 右侧下拉：全部/仅违规标记/仅我的回复）仅作者可见
- 作者昵称固定「一页君」+ 角标；普通用户「微信用户+随机 4 位」
- 不限字数；提交 30s 限频；违规跳文明公约页；作者侧已删条目灰显；展示留言总数「共 N 条」
- 深浅色全走 token，无硬编码

**三、🔔 认领任务**：
- **@前端开发（FE）** → **RQ-22-FE**：`pages/feedback/feedback.{js,wxml,wxss,json}` + `rules.{...}` + 设置页入口；验收 AC-01~12（设计稿 §7）
- **@全栈开发（FS）** → **RQ-22-FS**：`cloudfunctions/feedback/*`（create/list/delete）+ 关键词词库 + AI 审核；接口字段对齐（parentId/rootId/isAuthor/status），按 PRD §五；**AUTHOR_OPENID 环境变量**预留

**四、红线**：WXS 纯 ES5 + touch 禁 return false；深色走 token；icon 用 `/assets/icons/*.svg`（深色 `-dark` 后缀）。

> — 产品经理（PM）

---

### 📢 2026-08-07 19:25 【PM · RQ-22 PRD v1.0 确认 · 作者识别方案固化 · @产品设计师 可开工】

> **对象**：产品设计师（PD，认领 RQ-22-D）、owner（知悉）、全员
> **发布**：产品经理（PM）

**一、PRD 升级至 v1.0（已确认）**：`docs/02-产品设计/PRD-RQ22-意见反馈留言板.md`

**二、新增确认决策**：
- **作者识别**：openid 白名单 → 云函数环境变量 `AUTHOR_OPENID` · 单作者（owner）· 云端校验不可伪造
- **管理入口**：留言板页面内动态渲染（isAuthor 驱动）· 删除按钮 🗑 + 筛选入口仅 owner 可见 · 无独立管理后台
- **作者昵称**：固定为「一页君」

**三、PRD §4.3 已详细文档化**：含识别方案架构图、管理入口页面布局示意、安全性分析。

**四、🔔 @产品设计师（PD）**：可正式开工 **RQ-22-D**。设计时注意：留言板页面需为 owner 视角预留管理控件（删除按钮 + 筛选入口），通过 `isAuthor` 布尔值驱动显示/隐藏。

> — 产品经理（PM）

---

### 📢 2026-08-07 11:47 【PM · RQ-22 意见反馈留言板 PRD 就绪 · 提交 PD 设计 UI/UX · @产品设计师】

> **对象**：产品设计师（PD，认领 RQ-22-D）、owner（知悉）、全员
> **发布**：产品经理（PM）

**一、RQ-22 意见反馈留言板 PRD 已就绪**：`docs/02-产品设计/PRD-RQ22-意见反馈留言板.md`（v0.1 → 已升级 v1.0）

- 设置页「意见反馈」落地为留言板：留言/回复（多级嵌套楼中楼）/作者角标/筛选删除/合规审核（关键词黑名单+AI）/文明公约页
- 已确认决策：匿名自动昵称 · 仅限频（30s）· AI 失败仅黑名单兜底 · 云数据库存储 · **openid 白名单单作者 · 页面内管理入口**
- 验收标准 12 条（PRD §六）

**二、🔔 @产品设计师（PD）**：认领 **RQ-22-D**（UI/UX 设计）——留言板主页面 + 文明公约页的交互/视觉规范（列表/嵌套回复/作者角标/删除筛选/公约页），含暗色 token，输出 D-02 增量文档。任务见任务表「🆕 RQ-22 意见反馈留言板 · 任务跟踪」。

**三、流程**：PD 设计通过 → 评审 → FS 后端 + FE 前端开发 → 验收。

> — 产品经理（PM）

---
### 📢 2026-08-07 13:16 【owner 确认 RQ-22 设计 · 正式建 FE/FS 任务 · 必须严格按设计文件开发 · @前端开发 @全栈开发】

> **对象**：前端开发（FE，认领 RQ-22-FE）、全栈开发（FS，认领 RQ-22-FS）、owner（知悉）
> **发布**：产品设计师（PD）

**RQ-22 意见反馈留言板设计已确认（owner 13:16）**，任务已正式建单，请立即认领开工。

**一、任务**
- **RQ-22-FE**（前端开发）：`pages/feedback/feedback.*` + `rules.*` + 设置页入口改 `navigateTo`（AC-01~12 自测）
- **RQ-22-FS**（全栈开发）：`cloudfunctions/feedback/{create,list,delete}` + 关键词词库 + AI 审核（AUTHOR_OPENID 环境变量 / isAuthor 云端判定 / 软删除）

**二、实现依据（唯一依据，严格遵循，不得自行偏离）**
1. **开发交接单**：`docs/04-开发实现/RQ-22-意见反馈留言板-开发交接单-20260807.md`（FE §三 / FS §四）
2. **设计规范**：`docs/02-产品设计/D-02-增量-RQ22-意见反馈留言板-UIUX设计.md`（v1.0 · 全部 rpx 参数/交互/暗色 token/AC 12 条）
3. **PRD**：`docs/02-产品设计/PRD-RQ22-意见反馈留言板.md`（v1.0 已确认）
4. **交互 demo（1:1 参考）**：https://tengfeizhao1219.github.io/One-News/demo/feedback-demo.html

**三、红线**：SOP v1.6 §15.4（WXS 纯 ES5 + touch 禁 return false）· 深色双套 token 禁硬编码 · icon 用 /assets/icons/*.svg。实现与设计不符时先与 PD 对齐。
...
**owner 需求**：分享不局限于微信，用户可选择分享到其他平台/沟通工具。

**微信生态限制**：小程序无法编程调起其他 App 分享面板；跨平台分享靠「剪贴板 + 相册图片」实现。

**已实施**（`pages/detail/detail.wxml/js/wxss` + `components/share-card/share-card.js`）：
1. 底部「分享」按钮改为普通按钮 → 弹出自定义分享面板（半屏弹层，遮罩+动画）
2. 面板 3 个选项：
   - **微信好友/群**（`open-type="share"` 原生转发，onShareAppMessage 不变）
   - **复制内容**（`wx.setClipboardData` 复制「【一页】标题 + 分类 + AI 摘要 + 品牌尾」，可粘贴到微信/QQ/钉钉/邮件等任意平台）
   - **保存图片**（share-card 新增 `exportTempFile()` → `wx.saveImageToPhotosAlbum` 保存 AI 摘要图到相册，可发任意平台；相册权限引导授权，拒绝跳设置页）
3. `setNewsRetained` 上报从 onShareAppMessage 移到 `onShareTap`（面板打开即算分享意图，任一分享方式都保留新闻）

**验证**：v10 53/0、v7-reading-mode 通过；语法检查通过。

**真机验收**：点「分享」→ 面板弹出；微信好友/群转发正常；复制内容后粘贴；保存图片到相册。
...
**一、新需求**：**RQ-20 侧边栏「全部」分类**（聚合其他新闻分类）已录入需求池 v1.2，后续迭代推进。

- 侧边栏分类新增「全部」，**置于「推荐」上方**（全部 → 推荐 → 科技 → 国际 → 科学探索 → 社会）
- 「全部」数据范围：**含推荐**（全部分类聚合）；打开侧边栏首次默认「推荐」+ 记住上次；**首页首次加载仍默认「推荐」**
- 开发任务单：`docs/01-需求规划/侧边栏全部分类-开发任务单-2026-08-07.md`（FE-01/FE-02/FS-01/QA-01）

**二、⚠️ 与 DG-03 的关系澄清（重要，请勿误读）**：

- **DG-03 曾**「去除 all，改为推荐独立分类」（2026-08-06 数据治理最终结论）；
- **RQ-20 是增量调整，不是 DG-03 回退**：新增「全部」作为聚合视图**补充**，但**「推荐」仍独立保留**（首页默认 + AI 精选语义不变）；
- 后端 `getNewsList` 的 **all 分支本就保留**（DG-03 时未删，兼容旧分享链接），本次仅前端恢复请求 it；**零后端改动**；
- 两者并存逻辑：**推荐 = AI 精选要闻（首页默认）**，**全部 = 全分类聚合视图（侧边栏可选）**。

**三、✅ RQ-20 已闭环（2026-08-07 09:47）**：FE-01 ✅（`6c41fa3`，PM 代实现 constants.js all 置顶）+ FS-01 ✅（后端零改动可用）+ QA-01 ✅（回归 v5/v6/v7/v10/v11/v13 全绿 + FE-02 验证点全过 + 验收标准 7 项通过）。
> 🔔 **@owner**：云端实测 `getNewsList({category:'all'})` 返回全部分类新闻（FS-01 已给指引）+ 真机确认侧边栏「全部」展示；**@各角色**：RQ-20 验收通过，可发布生效。
...
> **对象**：所有角色（知悉）
> **发布**：产品设计师（PD，owner 18:28 决定）

owner 决定「暂时不讨论侧边新闻列表页重新设计方案」：
- ❌ **不建 FE 开发任务**（D-10 尚未建任务，本公告即取消动作）
- ⏸ D-10 设计文档与 demo **保留在仓库**供后续复用：`docs/02-产品设计/D-10-增量-侧边新闻列表页面-详细设计.md` + `demo/side-panel-demo.html`
- ✅ 首页 UI/UX 维持现状，无任何改动

### ✅ 2026-08-06 17:47 【owner 真机验收 · 全部待验项通过 · 001/006 + 004/005/005追修/007/008/009 + AB-01 + DG-01~05 全部关闭】

> **对象**：全栈开发（FS）、小程序前端开发（FE）、PM（知悉）、所有角色
> **发布**：产品设计师（PD，owner 17:47 真机反馈回填）

**owner 17:47 真机反馈**：「BUG-20260802-001 以及 BUG-20260802-006 以及全都通过」→ 全部待验收项真机通过，正式关闭：

- ✅ **BUG-20260802-001** 详情页翻页单向失效 —— 关闭（owner 真机通过 17:47）
- ✅ **BUG-20260802-006** 切分类 ~0.5s 提示 —— 关闭（owner 真机通过 17:47）
- ✅ **BUG-20260806-009** 状态栏不透明 + 按钮胶囊对齐 —— 关闭（owner 真机通过 17:47）
- ✅ **BUG-20260806-008** 侧边栏滚轮三优化 + 分类上移（面板滑动 v3）—— 关闭（owner 真机通过 17:47）
...
- ✅ **DG-01~05** 数据治理（云函数部署后真机验证：首页推荐 / 首屏 10 条 / 翻底 5×3 / 收藏纯本地 30 天）—— owner 真机通过 17:47

**剩余**：DG-06 全量回归测试待 PM 排期（不属于 owner 真机验收范围）。

### ✅ 2026-08-06 16:05 【FS 调整 · recommend 抓取量 10→8 条/轮（DG-01 超时实证）· commit `b2114dd` · @owner 部署复验】

> **对象**：owner（重新部署 refreshNews + 复验 recommend）、PM（R5 首屏 10 条口径确认）、FE（DG-03 适配）
> **发布**：全栈开发（FS）

**触发**：16:01 云端日志实证——recommend **10 条 prompt 触发智谱 50s 超时**（ZHIPU_TIMEOUT），总耗时 56.8s 逼近 60s 云函数上限（余量仅 3.2s，再慢几秒即超时不写入、首页变旧）。FS DG-01 评估中的超时预警被实证。

**调整**（owner 16:05 确认）：`RECOMMEND_COUNT` 10 → **8 条/轮**（智谱 ~45s 内完成，降级链余量充足）。

**🔔 联动确认**：
- **[PM]** R5「首页首次 10 条」口径：recommend 单轮 8 条 → 首屏 10 条需 DG-03 用「首屏 8 + 翻底拉取追加」实现，请确认口径或放宽为 8 条
- **[owner]** 重新部署 `refreshNews` 后观察 recommend 分类日志（智谱不再超时、条数=8）
- **DeepSeek 402 余额不足**（同日 2 次出现）：建议充值或确认降级策略（聚合兜底已保证不断供）

> — 全栈开发（FS）

---
### 📢 2026-08-06 16:00 【FE 交付 · BUG-20260806-008 滚轮交互三优化完成 · @PD 按 AC-RQ15-18/19/20 验收 / @owner 真机】

> **对象**：PD（AC 3 项代码级验收）、owner（真机体验：锚点/热区/跟手）、PM（知悉）
> **发布**：小程序前端开发（FE）

**BUG-20260806-008 · 滚轮交互三优化（owner 15:12 反馈）已完成**

**① 选中锚点修正（AC-RQ15-18）**：
- `category-wheel.js` `_updateTranslate`：`ty = (anchorIndex - idx) * itemHeight`（去掉原 `Math.max(0,·)` clamp）
- idx=0（全部）→ ty=72rpx（列表下移 1 行，顶部留 1 个空行空间，不贴顶）
- idx=1（科技）→ ty=0（选中项恒在第 2 行，顶部显示「全部」）
- 末项无溢出（5 项 × 72rpx，最大上移 216rpx，可视 6 行 432rpx 内安全）

**② 触摸热区全条带（AC-RQ15-19）**：
- `.wheel` 容器（128rpx × 6 行）触摸事件已全量覆盖空白/行间隙
- 热区外扩：`padding-right: 32rpx + margin-right: -32rpx`（视觉不变，命中区域向列表侧扩展）

**③ 拖动连续跟手 + snap（AC-RQ15-20）**：
- `onTouchMove`：位移 = `startTranslate + deltaY` 连续跟随手指（无按格跳变）；`rawIdx` 仅用于高亮/change/震动
- 越界：首/末项叠加弹性 bounce（max 18rpx，AC-RQ15-06 保留）
- `onTouchEnd`：松手 300ms 缓动 snap 最近分类（wxml transition 恢复）
- 一次 setData 合并位移 + bounce，减少渲染

**回归**：v511 40/0 ✅ / v7 63/0 ✅ / v13 88/0 ✅
**无回归**：选中态淡蓝字体无底色（v2.5）/ 震动 / active scale(1.08) / 可视 6 项 / 5 项分类

**🔔 @PD**：按 D-02 v2.6 AC-RQ15-18/19/20 代码级验收；**@owner**：真机体验滚轮（首类顶部留空 / 全条带可拖 / 连续跟手丝滑）。
> — 小程序前端开发（FE）

---
### 📢 2026-08-06 15:25 【PD 验收 · BUG-20260806-007 D-09 v1.2 系统级落地 AC 8 项全部通过 · @owner 真机回归 UAT-03】

> **对象**：owner（真机回归 UAT-03）、FE（验收通过）、PM（知悉）
> **发布**：产品设计师（PD）

**BUG-20260806-007（FE `7693b0d`）→ PD 代码级 ✅ 通过（AC-D09-01~08 全绿）**：
- 导航条带：5 页面 + home 统一 `top=menuTop / height=calc(menuHeight+24rpx)`（仅下扩 12px）
- 按钮 explicit 锚定胶囊中心：`top=calc(menuTop+menuHeight/2-32rpx/2)`（detail nav-back / settings nav-home 落地）
- 内容起始：`--nav-offset = menuTop+menuHeight+12` 根节点注入；detail +12rpx / 列表·次级页 +8rpx
- 进度条无底槽（transparent，胶囊下 12px）；正文无分割线保留；底部操作栏未动
- 回归：v13 ✅ / v7 63/0 ✅ / v511 40/0 ✅

**@owner 真机回归 UAT-03**：① 详情页标题位于导航栏下 12rpx（6px）；② 导航条带与胶囊呼吸 12px、按钮仍锚定胶囊中心；③ 进度条无底槽（仅浅蓝线）；④ 收藏/历史/设置/关于首屏顶部元素不被遮挡。
...
---
### 📢 2026-08-06 15:15 【PD · 008 滚轮交互三优化提单（owner 15:12 反馈）+ D-08 深色复验通过闭环 · @FE 处理】

> **对象**：FE（008 修复）、owner（D-08 已闭环；008 修复后真机）、PM（知悉）
> **发布**：产品设计师（PD）

**owner 15:12 反馈**：
1. **005 追修本身没问题**（去底色 ✅）；
2. **D-08 深色复验：已通过** → D-08 正式闭环（G-07 残留 51→0，豁免 9 + 深色专用覆盖 4 处）；
3. **DG-01~05 真机验证：待验证**（owner 部署云函数后验：首页默认推荐 / 首屏 10 条 / 翻底 5×3 / 收藏纯本地 30 天）；
4. **滚轮还需 3 点提升** → **PD 提单 BUG-20260806-008 @FE**：
   - ① **选中锚点修正**：分类应位于上部，选中项恒在容器第 2 行、顶部恒留 1 个分类名空间（首类留空行）——**代码根因已定位**：`_updateTranslate` 的 `Math.max(0, idx-anchorIndex)` 使 idx≤1 时选中项贴顶，改 `ty=(anchorIndex-idx)*itemHeight`；
   - ② **触摸热区**：整个滚轮条带（128rpx×6 行）+ 合理外扩都可触摸滚动，不限于分类文字命中；
   - ③ **滚动连续性**：拖动时位移连续跟随手指（非按格跳变），松手 300ms 缓动 snap 最近分类，越界 bounce 保留；
5. D-02 已升 **v2.6**（AC-RQ15-18/19/20 新增），验收标准见 Bug 台账 008。
...
---
### ✅ 2026-08-06 13:22 【owner 真机验收通过 · BUG-20260806-006 收藏/浏览历史同步问题已关闭】

> **对象**：FE（交付闭环）、PD（台账状态同步）、PM（知悉）、owner（确认）
> **发布**：小程序前端开发（FE）

**owner 反馈**：「上面关于收藏、浏览历史的问题验收通过了，可以关掉了」

**BUG-20260806-006 · 收藏后列表不显示新条目 → ✅ 正式关闭**
- 修复：FE `5b87b1d`（方案 1 全局单例：detail/favorites/history 统一 `localCache`，容量 200→500）
- PD 代码级验收 ✅（13:20）+ **owner 真机验收通过（13:22）**
- 验证覆盖：收藏→列表立即出现 / 取消→立即消失 / 浏览历史同源生效

**闭环状态**：BUG-001/002/005追修/006 + UAT-01/02/04/05 均已通过；剩余 UAT-03（标题顶置，D-09/BUG-007 落地后回归）。
...
**背景**：首页右上角（胶囊下方）设置按钮为 BUG-20260806-004 导航对齐时误加——wxml 绑定 `onSettings` 但 home.js 无此方法（**死按钮，点击无响应**）；且与 ⚙ dock 菜单设置入口功能重复。

**处理（owner 2026-08-06 13:21 裁定「移除吧，这个没什么用」）**：
- `home.wxml`：移除 `top-bar-settings` 按钮；`fixed-top-bar` 保留为空占位（维持胶囊对齐结构，pointer-events:none 不拦截卡片手势）
- `home.wxss`：移除 `.top-bar-inner` / `.top-bar-settings*` / `.page--dark .top-bar-settings` 样式
- `settings.svg` 图标**保留**（dock 菜单仍在用）
- 设置入口现状：仅 ⚙ 浮动「更多功能」菜单 →「设置」（唯一入口）

> — 小程序前端开发（FE）

---
### 📢 2026-08-06 13:20 【PD 验收 · 006 / 005追修 / D-08 方案A 代码级全部通过 · @owner 真机确认 3 项】

> **对象**：owner（真机确认 3 项）、FE（验收通过）、FS（D-08 方案 A 通过）、PM（知悉）
> **发布**：产品设计师（PD）

**一、BUG-20260806-006 收藏列表不显示（FE `5b87b1d` 方案 1 全局单例）→ PD 代码级 ✅ 通过**
- detail/favorites/history 三处 `new LocalCache` → 统一全局单例 `localCache`；单例容量 200→500
- 全项目业务代码 `new LocalCache` **零残留**；回归 b06 20/0 ✅ / v13 ✅
- **@owner 真机确认**：收藏→收藏页立即出现；取消→立即消失

**二、BUG-20260806-005 追修（FE `9bb5b60` 去背景高亮）→ PD 代码级 ✅ 通过**
- `.wheel-label.active` 移除 background-color + border-radius，仅保留淡蓝字体；transition 清理
- `--wheel-bg-selected` token 保留（v13 契约不破坏）；js/wxml 零改动
- **@owner 真机确认**：选中项淡蓝字体、无背景高亮、触摸放大不变

**三、D-08 方案 A（FS `639746b` 51 处兜底删除）→ PD 代码级 ✅ 通过**
- 8 文件 +35/-213；深色专用覆盖保留（浮层 + 按压态 ×2）
- `--color-warning-subtle` token 双变量块值一致 + 2 处琥珀改 var()；v13 88/0 ✅
- **@owner 真机复验**：深色模式首页菜单浮层 / 收藏按压反馈 / 深色整体视觉无回归

**⚠️ 待同步**：D-02 v2.4「淡蓝底色保留」描述将更新为「去底色」（005 追修后规范已变），PD 跟进。
...
**BUG-20260806-006 · 收藏后列表不显示新条目 · FE 已修复（commit `9bb5b60`）**

**根因**（PD 已定位）：detail/favorites/history 各自 `new LocalCache({maxItems:500})` 独立实例 → 各自独立 `_memory` Map；`get()` 内存命中即返回（不查 Storage）→ favorites 页实例内存旧值遮挡 detail 页新写入（另一实例 + Storage）。

**修复（方案 1 · 全局单例）**：
- `pages/detail/detail.js` / `pages/favorites/favorites.js` / `pages/history/history.js`：三处 `new LocalCache(...)` → 统一引用 `require('../../utils/localCache').localCache` 全局单例
- `utils/localCache.js`：单例容量 200 → **500**（与 detail 引擎缓存规模匹配：7 分类列表 + 各分类详情缓存 + favorites + browseHistory），TTL 7 天
- 全项目业务代码现仅剩单例一处实例化（`grep new LocalCache` 仅命中单例定义）

**验证**：
- 单例一致性模拟：detail 写 → favorites 读立即命中 ✅ / 取消后读取立即为空 ✅
- b06-localcache 单测 **20/0 通过**（含容量改动后 LRU 行为）
- v12 收藏回归 **31/2**：2 失败为已知断言过期（004 已静默移除「待同步」toast，@PM 待同步 v12 断言，非本次引入）

**验收标准对照**（Bug 台账）：
- 详情页收藏 → 返回收藏页 → 新条目立即出现 ✅（单例内存共享）
- 详情页取消收藏 → 收藏页对应条目立即消失 ✅
- 首页侧边栏收藏列表与收藏页数据一致 ✅（home 本就使用同一单例）
- 浏览历史列表同源修复 ✅（history 同用单例）
- 无回归：收藏 200 容量 / ☁️ 角标 / 云同步入队重试 / toast 提示不变 ✅

> **@PD**：按 Bug 台账验收标准代码级验收；**@owner**：真机验证「收藏→列表立即出现」。
> — 小程序前端开发（FE）
...
---
### 📢 2026-08-06 11:41 【PM · 数据治理任务分配提请 · 建议分工已列 · 请技术负责人（FS）正式拍板分配】

> **对象**：技术负责人（FS，拍板分配）、FE（承接 3 项）、PM（承接测试）、owner（知悉流程）
> **发布**：产品经理（PM）

**流程落位**：需求文档 ✅（`docs/01-需求规划/数据存储与展示统一整理-需求说明-20260806.md` + 最终结论说明）→ FS 技术评估 ✅（`docs/04-开发实现/数据治理-5项FS评估-20260806.md`）→ **任务分配 ⏳ 由技术负责人（FS）按评估结论正式拍板**。

**PM 建议分工（已填入任务表，待 FS 确认/调整）**：

| ID | 任务 | 建议负责人 | 依据 |
|----|------|-----------|------|
| DG-01 | 云函数改造（stale 空时兜底 + recommend 10 条/轮 + 停用 4 云函数） | **FS** | 后端唯一可做 |
| DG-02 | 详情引擎重构（砍补拉/跨分类跳转/来源识别/总数锁定/边界 toast/detail 去云端化） | **FS** | 引擎核心，评估方案已定 |
| DG-03 | 首页改造（推荐默认 + 10+5×3 + toast + 侧边栏） | **FE** | 纯前端 |
| DG-04 | 历史/收藏纯本地化（本地存储 + 透传来源 + 每日清理） | **FE** | 纯前端 |
| DG-05 | about 文案勘误（1 行） | **FE** | 顺手 |
| DG-06 | 测试（一致性/来源顺序/拉取计数/本地存储断言） | **PM** | 测试归属 |

**🔔 @技术负责人（FS）**：请基于 `数据治理-5项FS评估-20260806.md` 结论确认上述分工（或调整后拍板），确认后各角色即可认领启动。依赖关系：DG-03/DG-04 依赖 DG-01；DG-02 依赖 DG-01。
...
| UAT-20260806-03 | 真机验证 003 · 导航按钮层级 | owner | 🔴 | ✅ 通过（owner 17:47 回填）| FE `7693b0d` ✅ | 详情页 ‹ 返回→收藏/历史；about/settings 🏠→首页；**标题位于导航栏下 12rpx + 胶囊呼吸 12px + 进度条无底槽** |
...
| BUG-20260806-005 追修 | 滚轮选中态去背景高亮（仅保留淡蓝字体）| FE | 🟡 | ✅ **关闭（owner 真机通过 · 17:47）**| PD `33c4166` ✅ | 选中项淡蓝字体，**无背景高亮**；触摸放大/锚点/项数不变 · owner 真机确认通过 |
...
| BUG-20260806-007 | D-09 v1.2 系统级落地：导航条带胶囊呼吸（仅下扩 12px）+ 按钮锚定胶囊中心 + 内容起始位置（detail 12rpx / 列表 8rpx）+ 进度条无底槽 | FE | 🔴 | ✅ **关闭（owner 真机通过 · 17:47，UAT-03 同步回填）**| D-09 v1.2（owner 13:09 确认）| 6 页面统一实施；PD AC 8 项通过 + owner 真机通过 |
...
|----|------|--------|:---:|:---:|------|------|
| BUG-20260806-004 | 收藏 toast 静默化（去掉「待同步」）+ 全页面导航栏与原生胶囊对齐 + 标题/元信息固定相对尺寸 rpx + 正文区去分割线 + 底部操作栏分割线收窄 | FE | 🔴 | ✅ **关闭（owner 真机通过 · 17:47）** | owner 2026-08-06 09:22 需求 | **PD 代码级验收通过**（10:40）+ owner 真机通过 |
| BUG-20260806-005 | 侧边栏分类滚轮（panel-wheel）选中态字体淡蓝 `--primary`（owner 08:02 裁定 · 原 PD 编号 004 因冲突改号）| FE | 🟡 | ✅ **关闭（owner 真机通过 · 17:47）** | PD 08:02 提单 + D-02 v2.4 | **PD 代码级验收通过**（10:40）+ owner 真机通过 |
...
| **AB-01** | **✅ dock「关于一页」页面开发（FE 已交付 `34d20a5`）** | **小程序前端开发（FE）** | ✅ | PM 方案已确认 | 8 文件全部完成：`pages/about/about.{js,wxml,wxss,json}` + `home.{wxml,js}` dock 改造 + `info.svg` + `app.json` 注册；验收清单已就绪：`docs/05-测试验收/AB-01-关于一页验收清单.md`；✅ owner 真机验证通过（17:47） |
...
| **BUG-20260802-001** | **详情页翻页单向失效（上滑下一页/下拉上一页仅其一生效）** | 全栈开发 | 🟡 P1 | ✅ **关闭（owner 真机通过 · 17:47）** | 2026-08-04 FS 核实：detail.js 已同时存在 `_measureScroll` SelectorQuery 真实高度 + `bindscrolltolower/bindscrolltoupper` 原生边界事件 + `_bottomScrollTop` 校准防误复位。代码层面已覆盖原根因，owner 真机复测通过（17:47）。源：`docs/05-测试验收/Bug清单模板.md` |
...
| **BUG-20260802-006** | **卡片页/详情页切换分类时需有 ~0.5s 新分类名称提示** | 全栈开发 + 产品设计师 | ⚪ P3 | ✅ **关闭（owner 真机通过 · 17:47）** | 2026-08-04 FS 修复：`onWheelChange` 滚轮切分类补 `_showCategoryHint`；`_showCategoryHint` 双重 `wx.hideToast`（即时+50ms延迟）防止 toast 覆盖；展示时长 500ms→600ms。代码层面已修复，owner 真机复测通过（17:47）。|
...
## 🆕 RQ-20 侧边栏「全部」分类 · 开发任务跟踪（2026-08-07 启动）

> **启动**：2026-08-07（PM 建单）| **需求池**：RQ-20（v1.2）| **用户决策**：全部含推荐 · 首次默认推荐+记住上次 · 首页仍默认推荐
> **任务单**：`docs/01-需求规划/侧边栏全部分类-开发任务单-2026-08-07.md`
> **⚠️ 与 DG-03 关系**：增量调整非回退——推荐仍独立保留，新增「全部」为聚合视图补充（详见广播区 09:02 澄清）

### 任务链（依赖顺序）

| ID | 任务 | 负责人 | 优先级 | 状态 | 依赖 | 交付物/验收 |
|----|------|------|:---:|:---:|------|------|
| **FE-01** | `utils/constants.js` CATEGORIES 首位插入 `{ id:'all', name:'全部' }`（`currentCategory`/`panelCategory` 保持 recommend 不变） | 前端开发（FE） | 🔴 P0 | ✅ 已完成（`6c41fa3`：PM 代实现，all 置顶） | 无 | 常量变更 + commit |
| **FE-02** | ~~分类切换/侧边栏验证~~（**已合并至 QA-01**：验证点并入 PM 验收，2026-08-07 owner 裁定） | ~~前端开发（FE）~~ | ~~🔴 P0~~ | 🔀 已合并 | FE-01 | — |
| **FS-01** | 云端实测 `getNewsList({category:'all'})` 返回全部分类新闻（预计零代码改动） | 全栈开发（FS） | 🔴 P0 | ✅ 已验证（09:10：代码级 + mock 全通过，零改动；云端实测指引见 COMMLOG） | 无 | 云端验证记录 |
| **QA-01** | 回归验证 v7/v10/v11/v12/v13 + 验收标准 7 项 + **FE-02 并入的验证点**（all 加载、侧边栏首次默认推荐+记住上次、从 all 进详情正常） | 产品经理（PM） | 🟡 P1 | ✅ 已完成（09:47：v5/v6/v7/v10/v11/v13 全绿 + FE-02 验证点全过 + 验收标准 7 项通过） | FE-01 + FS-01 | 回归报告 + 验收结论 |

> 🔔 **RQ-20 已闭环（2026-08-07 09:47）**：FE-01 ✅ + FS-01 ✅ + QA-01 ✅ 全部完成，验收通过。剩余动作：owner 云端实测 all 分支返回数据（FS-01 已给指引）+ 真机确认侧边栏「全部」展示。
> 📄 完整验收标准见任务单 §四（分类顺序/首次默认推荐/记住上次/首页默认推荐/全部加载/详情阅读/回归全绿）。

---

## 🆕 RQ-22 意见反馈留言板 · 任务跟踪（2026-08-07 建单）

> **启动**：2026-08-07（PM 建单）| **需求池**：RQ-22（v1.5）| **PRD**：`docs/02-产品设计/PRD-RQ22-意见反馈留言板.md`（**v1.1 已确认**）| **设计**：`D-02-增量-RQ22-意见反馈留言板-UIUX设计.md`（v1.0 ✅ owner 13:11 确认）
> **owner 决策**（全部确认）：匿名自动昵称 · 多级嵌套回复 · 仅限频（30s）不限字数 · AI 失败仅黑名单兜底 · 云端存储（云数据库）+ 关键词黑名单+AI 校验 · **openid 白名单单作者（AUTHOR_OPENID 环境变量）· 页面内管理入口（isAuthor 驱动）· 作者昵称「一页君」· 不限字数 · 筛选 全部/仅违规标记/仅我的回复 · 删除软删除留痕（作者侧灰显）· 展示留言总数**
> **流程**：PRD ✅ → PD 设计 ✅（owner 13:11 确认）→ **开发（FS 后端 + FE 前端 · 严格按设计文件）** → 验收
...
>
> ⚠️ **owner 指令（13:16）**：RQ-22 设计已确认，任务已正式建单。**必须严格按设计文件开发**——设计文档 v1.0 + 开发交接单（含全部 rpx 参数/接口/AC 12 条/红线）为唯一实现依据，与设计不符需先与 PD 对齐，不得自行偏离。
...
| RQ-22-D | 意见反馈留言板 UI/UX 设计（v0.2 + demo） | 2026-08-07（owner 13:11 确认） | 产品设计师 |
