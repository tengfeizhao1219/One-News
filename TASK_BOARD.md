...
| **交互设计师** | UX-B1 | **走查报告重跑 v1.2** —— 你的 v1.1 基于被 QA 回退污染的 main（`3515e5a`）；`nav-back`/`back-bottom`/`pager-hint`/`boundary-chip`/`detail-source-link` 现已全部移除，「BUG-005 已修复(220rpx)」结论已失效 | 🔴 | 已就绪 | 继承 |
| | UX-B2 | **RQ-15-D** 侧边分类滚轮交互增量（下迭代第一棒，PRD 已就绪 10 条 AC） | 🔴 | 无 | 继承 |
| | UX-B3 | 跟踪走查 8 项偏差关闭（与 FE-B4 对齐） | 🟡 | FE-B4 | 继承 |
