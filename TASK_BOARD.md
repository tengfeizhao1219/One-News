...
- 🔴 **[前端开发] 重开 3 项，给精确修复靶点（勿再以"已实现"闭环）**：
  ...
  - **更新文档**：`docs/04-开发实现/开发自测清单.md`、`设计走查清单.md` 补/修订上述 3 项用例。
- 📢 **[产品/视觉/交互/测试] 文档同步指令：「每分类默认 15 条新闻」**（代码已落地，PRD/设计/用例须对齐）：
  - 🟡 **产品**：PRD 补"每分类默认 15 条、分页加载"规则（关联既有需求）。
  - 🟡 **视觉/交互**：设计规范/走查清单标注 15 条分页与"加载更多"态。
  - 🟡 **测试**：补"每分类 15 条上限 + 分页加载"测试用例（并入 Q-05 验收 / Q-新系列）。
  - 🟡 **前端**：`docs/04-开发实现/` 注释/自测清单注明 15 条上限（代码 `MAX_NEWS=15` 已存在）。
...
| **BUG-20260802-004** | **详情/卡片页滑动后，侧栏"正在阅读"高亮标题未同步最新阅读项** | 前端开发 | 🟢 P2 | 🔴 重开 | 🆕 2026-08-02 用户直报。**⚠️ 2026-08-02 17:07 用户真机复测确认卡片 newsList 与侧栏 filteredNewsList 完全分叉（"之前已提过"=BUG-20260801-007/008 未真修）**。根因：`loadNews` 与 `loadPanelNews` 两次独立请求 + 独立 `_panelCache`，仅同步高亮不够。**修复（方案B）**：侧栏复用同一 `newsList`，移除独立 fetch/cache。**重开 007/008**。 |
...
| **BUG-20260802-006** | **卡片页/详情页切换分类时需有 ~0.5s 新分类名称提示** | 前端开发 + 交互设计师 | ⚪ P3 | 🔴 重开 | 🆕 2026-08-02 用户直报。**⚠️ 2026-08-02 17:07 用户真机复测：0.5s 提示未出现，反见"刷新中"加载更多同类新闻**。根因：`_showCategoryHint` 疑似被刷新/加载 UI 遮挡或时序覆盖，未真正渲染。**修复**：FE 真机验证 hint 实际渲染（z-index/时序），确保分类切换时 0.5s 提示可见；"每分类默认15条"(loadMoreNews MAX_NEWS=15) 已落地，需同步文档。 |
...
> **当前状态**：阶段一 ✅ | 阶段二 ✅ | 阶段三 ✅ | 阶段四 ✅ | **阶段五 🔴 Q-05 验收中** | Q-02/Q-03 ✅ | **BUG-P0-010 已止血待复测** | 🆕 **BUG-P1-011 分类契约不一致待产品裁定** | 全员任务 v2 已指派（见广播区）| B-08/B-11/B-12 ❌ 随 v4.2 作废 | **下迭代 🚀 RQ-15 全链路推进中（RQ-15-D ✅ / UI·T·FE ⏳）** | 🆕 **用户直报 6 Bug · #1/#4/#6 真机复测未过已🔴重开（17:07）+「每分类默认 15 条」全文档同步指令已下发（见 17:07 广播）**
...
> 🔴 **17:07 真机复测结论（覆盖 15:46 乐观判定）**：用户（产品 owner）真机复测 #1/#4/#6 **仍失败**，此前"已实现"判定有误。**#1 详情翻页单向**：`detail.js` `_clientHeight=windowHeight-150` 估算 + `onContentScroll` 触底阈值失真，真实 scroll-view 高度>估算则某方向触底永不成立→单向翻页。**#4 卡片≡侧栏数据完全分叉**（用户称"之前已提过"=BUG-20260801-007/008 未真修）：`loadNews`(卡片 `newsList`) 与 `loadPanelNews`(侧栏 `filteredNewsList`) 两次独立请求+独立 `_panelCache`。**#6 0.5s 提示未现**（反见"刷新中"+加载更多）：`_showCategoryHint` 疑似被加载 UI 遮挡/时序覆盖。**处置**：#1/#4/#6 🔴 重开并给精确修复靶点（见 17:07 广播）；**重开 BUG-20260801-007/008**；**「每分类默认 15 条」(`loadMoreNews` `MAX_NEWS=15`) 已成产品需求，指令产品/视觉/交互/测试/前端同步所有文档**（见 17:07 广播 §文档同步指令）。
> **最后更新**：2026-08-02 17:07 | **维护者**：项目经理 | **版本**：v4.19（🔴 3 项真机复测失败已重开 + BUG-20260801-007/008 重开 + 「每分类默认 15 条」全文档同步指令下发 + 15:46 乐观判定修订）
