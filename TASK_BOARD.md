...
✅ dock 排列：**浏览记录 → 我的收藏 → 关于一页 → 设置**（设置挪至最末）

**二、任务已创建**：AB-01（📋 待 FS 认领），需求文档：`docs/01-需求规划/关于一页-页面需求说明-20260805.md`
...
| **AB-01** | **🆕 dock「关于一页」页面开发** | **全栈开发** | 📋 | PM 方案已确认 | 8 文件：`pages/about/about.{js,wxml,wxss,json}` + `home.{wxml,js}` 改 dock + `info.svg` + `app.json` 注册；详见 `docs/01-需求规划/关于一页-页面需求说明-20260805.md` |
