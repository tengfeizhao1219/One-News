...
| | **UI-B8** | 🔴 **🆕 AI 摘要胶囊视觉设计**（owner 2026-08-04 15:41 需求，方案 A 已定）：① 首页卡片摘要区左上角「AI 摘要」胶囊（蓝紫渐变底，白色文字）；② 日间/暗色两态色值；③ 与卡片流布局协调（胶囊不撑高卡片、不挤标题、摘要正常排版）；④ 胶囊可点击与否待 owner 定（建议仅视觉提示，不点击）。**设计输入**：owner 方案 A 草稿（已展示）+ `D-02-增量-UI-B7-列表页重新设计.md`（首页卡片流规格）；**设计通过后 TL-B18 胶囊前端方可落地** | 🔴 | 无 | 🆕（PM 指派 2026-08-04 15:47） |
