...
| | PD-B4 | **🆕 owner 直连：更新 PRD/产品文档** — 卡片页元信息位置已从「标题→摘要→元信息」改为「标题→元信息→摘要」（与详情页布局一致）。请更新 `docs/02-产品设计/PRD-*.md` 或产品文档统一库中卡片页布局描述，标注此变更 | 🔴 | 无（前端已交付） |
| | PD-B5 | **🆕 owner 直连：更新 PRD/产品文档** — 元信息（卡片/详情）及操作栏（收藏/分享）字号已强制锁定 **22px**，不参与字体缩放。请在 PRD/产品文档中标注此红线（后续禁止修改，如需调整须 owner 确认） | 🔴 | 无（前端已交付） |
...
**🔴 产品经理（PD-B4）**：请更新 PRD/产品文档中卡片页布局描述，标注此变更（标题→元信息→摘要）。**已在全员任务指派表中创建 PD-B4。**
