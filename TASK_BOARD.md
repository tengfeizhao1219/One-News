...
- 🟡 **[产品经理]**：BLK-09 设备就绪后，对 #1/#4/#6 真机回归（重点 #1 双向翻页、#4 卡片≡侧栏数据源一致）。
...
| **全栈开发** | FE-B1 | **P0 翻页真机自测回报**（BUG-P0-010 已合入 main，`bindscroll` 已恢复）：上滑触底翻下条 / 下滑触顶翻上条 / 首末条边界 | 🔴 | 已就绪 | 继承 |
...
| **全栈开发** | ~~BE-A1/A2~~ | ~~B-08 / B-11~~ | — | — | ❌ **撤回**（文件已不存在） |
...
| **全栈开发** | TL-A1 | **回复 `feature/be-b13-b14` 是否废弃**（你 v4.10 已判 B-13❌/B-14❌）。**逾期未回复 PM 按废弃删分支** | 🔴 | 无 |
...
|----|------|------|:---:|:---:|------|------|
| **RQ-15-D** | 交互设计增量：侧边滚轮视觉/交互规范 | 产品设计师 | ✅ | ✅ | 无（PRD 就绪） | `docs/02-产品设计/D-02-增量-侧边分类滚轮.md` |
| RQ-15-UI | 暗色模式适配规范 | 产品设计师 | 🟡 | ⏳ | RQ-15-D | 设计规范增量（暗色 token） |
| RQ-15-T | 技术可行性评估（T-03 增量） | 全栈开发 | 🟡 | ⏳ | RQ-15-D | 技术方案增量（scroll-snap / 手势隔离） |
| RQ-15-FE | 前端实现：category-wheel 组件 | 全栈开发 | 🔴 | ⏳ | RQ-15-T | `components/category-wheel/` |
...
| ID | 阻塞原因 | 谁来解决 | 记录时间 |
|----|----------|----------|----------|
| BLK-01 | **BUG-P1-011 分类契约不一致**（农业/科学 tab 无数据源）待产品裁定「后端补类 or 前端下架」；**此裁定同时即为 B-09 Q2 答案** | 产品经理（PD-B1）→ FE-B6 / BE-B1 → 全栈开发 | 2026-08-02 |
