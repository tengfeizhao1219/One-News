...
**三、🚀 开发已启动（2026-08-07 09:04）**：RQ-20 已建单，见任务表「🆕 RQ-20 侧边栏全部分类 · 开发任务跟踪」——**FE-01（前端）+ FS-01（后端）📋，QA-01（PM）验收**。**FE-02 已合并至 QA-01**（09:40 owner 裁定：验证点并入 PM 验收，不再单独开发）。
> 🔔 **@前端开发（FE）**：认领 FE-01（constants.js 加 all，1 行）；**@全栈开发（FS）**：FS-01 ✅ 已完成；**@产品经理（PM）**：FE-01 后执行 QA-01（回归 + FE-02 验证点）。
...
| **QA-01** | 回归验证 v7/v10/v11/v12/v13 + 验收标准 7 项 + **FE-02 并入的验证点**（all 加载、侧边栏首次默认推荐+记住上次、从 all 进详情正常） | 产品经理（PM） | 🟡 P1 | 📋 待认领 | FE-01 + FS-01 | 回归报告 + 验收结论 |

> 🔔 **@前端开发（FE）**：认领 FE-01（5min，constants.js 加 all）；FE-02 已合并至 QA-01 不再单独开发；**@全栈开发（FS）**：FS-01 ✅ 已完成；**@产品经理（PM）**：FE-01 完成后执行 QA-01（回归 + FE-02 并入验证点）验收。
