...
---
### 🎨 2026-08-04 17:35 【PD 交付 UI-B8 AI 摘要胶囊 v1.0 · ⚠️ 需 owner 裁定视觉方案 · TL-B18 前置】
...
> | **全栈开发** | **TL-B18** | AI 摘要链路修复 + 胶囊前端实现（①`summarySource` 字段/优先级/DashScope 重试——**PM 已先行实现 commit `655ffa1`，请 review 确认**；②按 UI-B8 设计稿落地胶囊 `home.wxml`/`home.wxss`） | 🔴 | UI-B8 |
...
> | **TL-B15** | **全栈开发** | **RQ-17 全局返回主页入口实现**：① 详情页导航栏新增主页图标（`wx.reLaunch` 跳转首页）；② 浏览记录页/收藏列表页默认含图标；③ 防抖 + 暗色适配 + 边界处理；④ 建议抽取 `components/nav-bar/` 组件 | 🔴 P0 | **UI-B6 设计通过（前端展示部分）**；后端无依赖 |
>
...
| **全栈开发** | TL-B1 | **回复 `feature/be-b13-b14` 去留**。**逾期未回复 PM 按废弃删分支** —— ⚠️ **2026-08-04 PM 再次催办（逾期红线）** | 🔴 | ⚠️ 催办中 | 继承 |
| | TL-B2 | 🆕 **确认 B-08/B-11/B-12 是否随 v4.2 一并作废**，并把 B-08~B-14 整批做一次「对齐 v4.2 架构」的清理裁定 —— ⚠️ **2026-08-04 PM 再次催办（逾期红线）** | 🔴 | ⚠️ 催办中 | 🆕 |
...
| | BUG-TL17-001 | **category-wheel 组件完全缺失** → 按 v5.3-final 顶部锚定范式实现 `components/category-wheel/`：选中分类固定第二行（上方保留前一个分类名）、其他分类围绕固定位上下滚动、可视区 6 项、超界隐藏滑动滑出、snap 到第二行、`wx.vibrateShort`、激活态 scale(1.08) 温和放大且颜色不加深字号不增大、无分割线、idle 选中项淡蓝底色 `var(--highlight-bg)` + 颜色 .5 weight 500。引用：UI-B7 设计文档 §3.2 + RQ-15-D v2.3 §2 | 🔴 P0 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-002 | **列表项高亮用直角色条** → v5.3-final A 方案：当前阅读项**仅标题 `var(--primary)`**，去底色去边框无圆角（原圆角边框/B 方案淡底均已废弃）。引用：UI-B7 设计文档 §3.1 | 🔴 P0 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-003 | **侧边栏仍用横向 Tab** → 替换为左侧顶部锚定分类栏（同 RQ-15-D v2.3 交互）。引用：UI-B7 设计文档 §3.2 | 🔴 P0 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-004 | **fab 浮动按钮渐变+重阴影** → 纯色 `--tag-bg` 背景 + 细阴影 `0 2px 10px rgba(0,0,0,.08)`，图标 `--text-secondary`。引用：UI-B7 设计文档 §3.4 / RQ-15-D §9.1 | 🔴 P0 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-005 | **6 枚 SVG 图标未导出** → 按 UI-B7 图标规范导出 6 枚 `.svg` 至 `assets/icons/` 并落地（描边式 stroke，禁止 emoji/填充）。引用：UI-B7 图标规范文档 | 🔴 P0 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-006 | **详情页缺少底部滑动提示条** → 补固定提示条（⇅ + 「上滑阅读下一条」，9px opacity .45，底部 absolute 渐变透明→页面底色，不响应点击）。引用：UI-B7 设计文档 §3.5 | 🟡 P1 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-007 | **列表项标题字号 32rpx** → **14px**（v5.3-final 降档，weight 600，2 行截断）。引用：UI-B7 设计文档 §3.1 | 🟡 P1 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-008 | **面板宽度 82%** → **90%**。引用：UI-B7 设计文档 §3.1 | 🟡 P1 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-009 | **theme.json `--setting-panel-bg` 仍为 .92** → **.78**（亮 `rgba(250,249,247,.78)` / 暗 `rgba(13,13,13,.78)`），dock 容器同步。引用：UI-B7 设计文档 §3.4 | 🟡 P1 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-010~012 | **dock / 详情页主页 / 收藏分享图标仍为 emoji** → 全部替换为描边 SVG（CSS mask 或 `<image>` 引入 `assets/icons/` 导出资源），跟随主题 token 颜色，不硬编码。引用：UI-B7 图标规范文档 | 🟡 P1 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-013 | ~~面板缺少关闭按钮~~ → **v5.3-final 已移除关闭按钮**（右滑关闭即可），本项废弃，无需处理 | — | TL-B17 | ❌ 已废弃 |
| | BUG-TL17-014 | **nav-dots 未移除** → 删除首页右侧竖排导航点（滚轮已替代其位置/切换语义）。引用：UI-B7 设计文档 §3.2 | 🟡 P1 | TL-B17 | 📋 待认领（全栈开发） |
| | BUG-TL17-015 | **面板头部缺少副标题** → 补「当前分类 · N 条」副标题（如「科技 · 3 条」）。引用：UI-B7 设计文档 §3.1 | 🟢 P2 | TL-B17 | 📋 待认领（全栈开发） |
| | **TL-B18** | 🔴 **🆕 AI 摘要首页展示链路 + 胶囊前端实现**（owner 2026-08-04 15:41 需求）：① **review 并接管 PM 先行实现**（commit `655ffa1`：`summarySource` 字段 `'ai'/'desc'/'title'` + batchInsert 优先级 AI 优先 + DashScope 失败重试 + getNewsList/前端透传 `isAiSummary`）；② **按 UI-B8 设计稿落地胶囊**（`home.wxml` 摘要区条件渲染 + `home.wxss` 胶囊样式，日间/暗色两态）；③ 需重新部署 `refreshNews`/`getNewsDetail`/`getNewsList` 三个云函数后验证。**验收**：①首页卡片首次进入即显示 AI 摘要；②AI 摘要条目显示胶囊、非 AI 不显示；③summarySource 链路（refreshNews→getNewsList→前端）正确 | 🔴 | **UI-B8（胶囊视觉）**；PM 先行代码可先 review | 🆕（PM 指派 2026-08-04 15:47） |
...
| | **UI-B8** | 🔴 **AI 摘要胶囊视觉设计**（owner 2026-08-04 15:41 需求，方案 A 已定）：① 首页卡片摘要区左上角「AI 摘要」胶囊；② 日间/暗色两态色值；③ 与卡片流布局协调（胶囊不撑高卡片、不挤标题、摘要正常排版）；④ 胶囊可点击与否 → **PD 定为不可点击**（纯视觉提示）。<br>**✅ PD 已交付 v1.0（2026-08-04 17:35）**：`D-02-增量-UI-B8-AI摘要胶囊.md` + `docs/showcase/ui-b8-ai-summary-chip.html`（1:1 原型）。<br>**⚠️ 视觉方案需 owner 裁定**：PD 就「蓝紫渐变」提出异议并给出 4 方案对比 —— **A-1 主题色轻胶囊（PD 推荐）** / A-2 中性灰 / A-3 蓝紫渐变（PM 原议） / A-3′ 纯色醒目。理由见文档 §2。<br>**owner 确认后 TL-B18 胶囊前端即可落地** | 🔴 | 无 | ⏳ **待 owner 裁定视觉方案**（PD 已交付 17:35） |
...
> | **产品经理** | V5-PM-01 | 按 `docs/05-测试验收/测试用例_v5-天行方案.md` 组织 P0 验收 | 🔴 | 🔄 | V5-FS-01 | 代码审查 ✅ + **TC-F01/TC-P01 云端实测已回报（08-04）**：code:0/inserted:20/engine:tianxing/3224ms ✅；**TC-F04 ⚠️ international 0 条**（天行限流）→ 转 V5-FS-02。待 TC-F05/TC-C01~03 详情页验证 |
> | **产品经理** | V5-PM-02 | 评估天行新闻质量与清洗效果是否可接受 | 🔴 | 📋 | V5-PM-01 |
> | **产品经理** | V5-PM-03 | 确认最终保留的分类口径（当前 v5.0 映射了 7 个天行分类） | 🟡 | 📋 | V5-PM-01 |
> | **全栈开发** | V5-FS-02 | 修复：①国际分类 0 条（天行 world code=130 限流，串行 200ms 间隔偏短）；②`remove()` 不支持 limit（gradedCleanup 未生效） | 🔴 | 🆕 已建单 | V5-PM-01（实测回报）|
...
| RQ-15-T | 技术可行性评估（T-03 增量） | 技术负责人 | 🟡 | 🆕 **可认领** | RQ-15-D ✅ | 技术方案增量（scroll-snap / 手势隔离）—— PM 2026-08-04 派单，依赖已满足 |
| RQ-15-FE | 前端实现：category-wheel 组件 | 前端开发 | 🔴 | ⏳ | RQ-15-T + UI-B7 确认 | `components/category-wheel/` —— UI-B7 v4 待 owner 确认后范围敲定 |
