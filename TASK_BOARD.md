...
| B-16 | 收藏/分享新闻 30 天保留（`isRetained` + `setNewsRetained` 云函数） | 后端开发 | 🟡 | 📋 **BE-B2** | 无（TL 已决策，已激活 BE/FE 认领 §4） | TL 决策见 `docs/03-技术方案/B-16-新闻存储保留机制-技术决策.md`；BE 实现 news 集合 `isRetained` 标记 + `setNewsRetained` 云函数 + refreshNews 跳过保留文档 + 30 天自动清理，FE 配合展示保留态 |
