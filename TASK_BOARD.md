...
> **启动**：2026-08-07（PM 建单）| **需求池**：RQ-22（v1.5）| **PRD**：`docs/02-产品设计/PRD-RQ22-意见反馈留言板.md`（**v1.0 已确认**）
> **owner 决策**（全部确认）：匿名自动昵称 · 多级嵌套回复 · 仅限频（30s）不限字数 · AI 失败仅黑名单兜底 · 云端存储（云数据库）+ 关键词黑名单+AI 校验 · **openid 白名单单作者（AUTHOR_OPENID 环境变量）· 页面内管理入口（isAuthor 驱动）· 作者昵称「一页君」**
> **流程**：PRD ✅ → **PD 设计 UI/UX（当前阶段）** → 评审 → 开发（FS 后端 + FE 前端）→ 验收
...
> 📄 待 PD 输出：留言列表/嵌套回复/作者角标/删除筛选/公约页 的交互+视觉规范增量文档。
