...
> **决策**：PM 裁定**立即激活前端开发**修复 #1/#4/#6（含 #4 同源的 #007/#008）。理由：① 三项根因已精确定位、修复靶点已给，无依赖阻塞；② 用户（产品 owner）正在真机复测，反馈闭环可用；③ 上一轮"代码已提交即闭环"已被证伪（#1 根因 `_clientHeight` 估算从未被触达），须打破该模式。BLK-09 仅阻塞 QA，不阻塞 FE 修复与用户复测。

> 🔴 **[前端开发] FE-C1 · 立即认领并真实修复以下 3 项（勿再以"代码已提交"闭环，须附行为验证证据）**：
...
| UX-B6 | **全系统交互走查 + GitHub 展示页**（按最新设计语言审计 25 项交互，提 P0/P1/P2 修改建议；交付 `D-02-增量-全系统交互走查.md` + `showcase/index.html` 可交互预览） | ✅ | 无 | 已完成（走查覆盖 首页12/详情8/全局5 + RQ-15 待建；发现 `--primary` 悬空 / 「R」语义 / 顶栏热区 / §3 漂移 等；展示页镜像 theme.json Token，支持浅暗+字体档位+reduced-motion）<br>🔗 **已上线**：https://tengfeizhao1219.github.io/One-News/showcase/ （源码 `docs/showcase/index.html`，看板首页已挂入口）<br>**补充 F12**：元信息 22rpx 不随字号缩放 → 按 D-02 §0「先标准后例外」登记入 §6.4 例外表，**与 PD-B7 红线同源**，交互侧只登记不推翻 |
...
> **结论**：前端开发已执行。卡片元信息（`.card-meta`）、详情元信息（`.detail-meta`）、底部操作栏文字（`.bottom-bar-btn`/`.bottom-bar-label`/`.share-btn-reset`）全部锁定为 **22rpx**，移除 `calc(…*var(--font-scale))` 缩放，每处标注 `/* owner 直连强制：锁定 22px，禁止回退 */`。**已在全员任务指派表中为产品经理创建 PD-B5（更新 PRD/产品文档标注此红线）。**
...
| **BUG-20260802-006** | **卡片页/详情页切换分类时需有 ~0.5s 新分类名称提示** | 前端开发 + 交互设计师 | ⚪ P3 | 🔴 重开·催办FE-C1 | 🆕 2026-08-02 用户直报。**⚠️ 2026-08-02 17:07 用户真机复测：0.5s 提示未出现，反见"刷新中"加载更多同类新闻**。根因：`_showCategoryHint` 疑似被刷新/加载 UI 遮挡或时序覆盖，未真正渲染。**修复**：FE 真机验证 hint 实际渲染（z-index/时序），确保分类切换时 0.5s 提示可见；"每分类默认15条"(loadMoreNews MAX_NEWS=15) 已落地，需同步文档。FE-C1 真实修复+行为验证。 |
...
> **最后更新**：2026-08-02 17:24 | **维护者**：项目经理 | **版本**：v4.20（📢 激活前端会话 FE-C1 催办 #1/#4/#6 真实修复 + 重开 007/008 同源 + 红线重申：禁"代码已提交"闭环、须附行为验证 + 「每分类默认 15 条」全文档同步指令）
