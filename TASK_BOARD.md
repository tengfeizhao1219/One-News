...
| **BUG-20260802-001** | **详情页翻页单向失效（上滑下一页/下拉上一页仅其一生效）** | 前端开发 | 🟡 P1 | ✅ | 🆕 2026-08-02 用户直报。可能 BUG-P0-010 回归（`bindscroll` 已恢复但仍单向）。待真机复测定方向；FE-B1 复测闭环。源：`docs/05-测试验收/Bug清单模板.md` |
| **BUG-20260802-002** | **卡片页元信息（来源/时间）大小·位置·分隔符与详情页不一致** | 前端开发 + 视觉设计师 | ⚪ P3 | ✅ | 🆕 2026-08-02 用户直报。卡片 `-`(home.wxml:73) vs 详情 `·`(detail.wxml:37)，class 不同。视觉补统一规范 |
| **BUG-20260802-003** | **侧边栏新闻标题点击应跳到首页卡片页（而非详情页）** | 前端开发 + 产品经理 | 🟢 P2 | ✅ | 🆕 2026-08-02 用户直报（已澄清：侧栏→卡片，不跳详情）。`onPanelItemTap`(home.js:743) 改定位卡片；产品更新交互流程文档 |
| **BUG-20260802-004** | **详情/卡片页滑动后，侧栏"正在阅读"高亮标题未同步最新阅读项** | 前端开发 | 🟢 P2 | ✅ | 🆕 2026-08-02 用户直报。`_handleDetailReturn` 跨分类对 `panelCurrentIndex` 同步疑似错位 |
| **BUG-20260802-005** | **设置按钮入口应置于卡片右下角并带 3D 浮动效果** | 前端开发 + 视觉/交互 | ⚪ P3 | ✅ | 🆕 2026-08-02 用户直报。当前在顶栏右侧(home.wxml:12)，需移至卡片右下角浮动层 |
| **BUG-20260802-006** | **卡片页/详情页切换分类时需有 ~0.5s 新分类名称提示** | 前端开发 + 交互设计师 | ⚪ P3 | ✅ | 🆕 2026-08-02 用户直报。首页无提示；详情 `showCrossingCategory` 仅着色无可见提示 |
...
> **当前状态**：阶段一 ✅ | 阶段二 ✅ | 阶段三 ✅ | 阶段四 ✅ | **阶段五 🔴 Q-05 验收中** | Q-02/Q-03 ✅ | **BUG-P0-010 已止血待复测** | 🆕 **BUG-P1-011 分类契约不一致待产品裁定** | 全员任务 v2 已指派（见广播区）| B-08/B-11/B-12 ❌ 随 v4.2 作废 | **下迭代 🚀 RQ-15 启动（RQ-15-D 📋 待交互设计师认领）** | 🆕 **用户直报 6 Bug 已录入+指派（BUG-20260802-001~006）**
> 🆕 **文档修订通知已发（DOC-01~DOC-14）**：多份权威技术文档与 v4.2 代码脱节，TL 8 份 / QA 2 份 / PM 2 份 / 产品 1 份 / 交互 1 份，见广播区顶部
> 🆕 **用户直报 6 项缺陷（2026-08-02）**：BUG-20260802-001 详情翻页单向(P1)/002 元信息不一致(P3)/003 侧栏标题跳卡片页(P2)/004 侧栏高亮不同步(P2)/005 设置按钮右下角3D(P3)/006 分类切换0.5s提示(P3)。PM 代 QA 录入 Bug清单模板.md，已按角色指派并提示更新对应文档（前端6项 / 视觉 002+005 / 交互 005+006 / 产品 003 / 测试补用例）。
> **最后更新**：2026-08-02 | **维护者**：项目经理 | **版本**：v4.17（用户直报 6 Bug 录入+指派：BUG-20260802-001~006 + 广播区派发 + 各角色文档更新提示）
