...
| | **TL-B17** | 🔴 **🆕 UI-B7 列表页前端实现**（owner 2026-08-04 设计确认）：① 首页左滑手势面板（跟手位移，阈值 60px 打开，右滑或 × 关闭）；② 列表页左侧竖排分类滚轮（6 分类 + 收藏，选中项圆角高亮 chip，触摸实时切分类 + snap 吸附 + `wx.vibrateShort`）；③ 列表项仅标题（15px/600，2 行截断，无来源/时间）；④ 当前阅读项圆角边框高亮 + 点击定位回首页卡片；⑤ 列表页顶部「今日新闻 · N 条」头部 + 分类联动；⑥ 底部固定滑动提示条（首页 + 详情页）；⑦ 图标按规范导出 6 枚 `.svg` 至 `assets/icons/` 并落地；⑧ 同步 `theme.json` `--setting-panel-bg` 至 `.78`。**设计输入**：`D-02-增量-UI-B7-列表页重新设计.md` + `D-02-增量-UI-B7-图标规范.md`；**验收**：设计文档 §5 四态映射表。⚠️ **严格按设计文档精度实现（位置/尺寸/颜色/圆角/间距/图标/透明度），禁止自行推断。完成后标记「🔄 待 PD 验收」，PD 逐项走查，不合格提 Bug。** | 🔴 P0 | **UI-B7 设计通过** + RQ-15-D + UI-B6（dock） | 🔄 待 PD 验收（全栈开发已实现 2026-08-04，commit `a339822`） |
