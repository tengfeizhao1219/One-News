...
> **🔴 BUG-PD-017 · category-wheel 组件停留 v3.1，落后 owner 四轮裁定（8 项）｜P0｜→ 全栈开发**
>
...
> | # | 项 | 规范 | 现状 | owner 依据 | 级别 |
> |---|---|---|---|---|---|
> | 1 | **锚定位置** | 顶部**第二行** `ty = max(0, -(idx-1)×72rpx)` | **垂直居中** `ty=(3-idx)×72rpx`（`.js:46`） | v5.1 + v5.3-final「**一定要确保**固定在顶部第二行不随滚动移动」 | 🔴 **P0 红线** |
> | 2 | 线框 | 无 | `border: 1px solid var(--primary)`（`.wxss:43`） | v5.1「不需要蓝色线框」 | 🔴 |
> | 3 | 选中字色 | `rgba(0,0,0,.5)` | `var(--primary)`（`.wxss:40`） | v5.2「颜色不需要加深」 | 🔴 |
> | 4 | 字号/字重 | `24rpx`/`500` | `28rpx`/`600`（`.wxss:41-42`） | v5.2「字体也不需要太大」 | 🔴 |
> | 5 | 激活放大 | `scale(1.08)` | **完全缺失** | v5.2「滚动到固定位时有放大效果」 | 🔴 |
> | 6 | 上下指示线 | 已废除 | `.wheel-indicator-top/bottom` 残留（`.wxss:56-81`/`.wxml:21-22`） | v5「去掉分割线」 | 🟡 |
> | 7 | 可视项数 | 6 | `visibleCount: 7`（`.js:17`） | v5「默认 6 分类可视」 | 🟡 |
> | 8 | chip 内边距 | 无 | `padding: 0 12rpx`（`.wxss:45`） | v5.1 去线框即去 chip | 🟡 |
>
> **完整修复代码（JS / WXSS / WXML 三段，可直接套用）见验收报告 v1.4 §3**。#1 是 owner 两次明示、第二次加重语气的需求，**不得再遗漏**。
>
> **PD 自省**：v1.3 走查 #37 仅查底色 token、未下钻组件源码，低估问题范围。责任在 PD，非 FS（FS 按提单精确执行）。

> **🔴 BUG-PD-018 · v6.2 title 兜底导致同卡片标题重复渲染｜P1｜→ 全栈开发**
>
...
>
> **PD 裁定（补充定义）**：第三档**不渲染摘要区**（`displaySummary = ''` + `summaryParagraphs: []`），保持标题 + 元信息的干净形态。空数组时 `wx:for` 不产出节点，**无需改 WXML**。修复代码见报告 §4。
>
> ⚠️ **流程提醒**：PD 未定义过 title 档视觉，FS 自行推断违反《PD→FS 交付精度规范》——**未定义场景应回问 PD**。
>
> ✅ 同批次 `isAiSummary: summarySource === 'ai'` 收紧判定**正确**，胶囊不再误标 desc/title 档。
