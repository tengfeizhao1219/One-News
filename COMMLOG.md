...
| # | 修复项 | 代码证据（v6.3） | 状态 |
|---|--------|---------|:---:|
| ⑥ P0 | 正文串入新闻 | `contentFetcher.js` `locateBodyHtml`+`trimExtraneousContent`（L85/103/113）；`getNewsDetail/index.js`（L113/126/134） | ✅ |
| ① | 国际分类 0 条（限流） | `tianxing.js` 分类间延迟 500ms + 指数退避 500/1500/3000ms（L128/137/153/170，标注 V5-FS-02-①） | ✅ |
| ② | remove limit | `refreshNews/index.js` `batchCleanup` 改用 `.get()`→按 `_id` 分批 `.remove()`（L72/87，标注 V5-FS-02-②） | ✅ |
| ③ | 广告声明噪音 | `newsCleaner.js` `/^(广告声明|广告)[：:]/i` + 截断（L115/116/185/186，标注 V5-FS-02-③） | ✅ |
| ④ | 图片说明残留 | `newsCleaner.js` 图源/图片说明规则 + 短行图说启发式（L137/141/151/152，标注 V5-FS-02-④） | ✅ |
| ⑤ | 微信扫码引导 | `newsCleaner.js` `用微信扫描二维码`/`扫一扫分享` 规则（L95/96/98，标注 V5-FS-02-⑤） | ✅ |

### 二、处置
- V5-FS-02 标记 **✅ 已交付**；08-04 22:05「全部未交付」广播标 `[已处理]`（结论基于过期快照）。
- **V5 上线唯一硬阻塞解除**；剩余上线前置 = Q-02/Q-03 测试 + Q-05 验收 + 5.6 上线检查清单。
- ⚠️ 仍建议 owner/PM 在真机环境对 ⑥ 正文串入抽样（TC-C03）确认线上无残留。

### 三、同步文件
- `TASK_BOARD.md`：广播区新增 17:40 PM 复验广播 + 22:05 旧广播标 `[已处理]` + 底部「当前状态」更新至 v4.25。
- `CONTEXT.md`：关键进度补 V5-FS-02 行（✅）+ 当前待办刷新为 6 角色口径 + 最后更新 08-05。

### 四、找谁
- 项目经理（PJM，记录 + 复验）｜🔔 关注人：产品经理（推进 Q-02/Q-03）→ 产品设计师（BUG-PD-017/018 二次验收）→ owner（TC-C03 真机抽样）
