...
- **PM v1.1 变化**：owner 13:11 确认设计稿后，Q1~Q4 全部定稿：
  - Q1 不限字数（UI 不硬性截断）· Q2 筛选 全部/仅违规标记/仅我的回复 · Q3 软删除作者侧灰显 · Q4 展示「共 N 条」
  - 设计文档/交接单已由 PM 同步更新（Q 标注），但交接单 §六 标题仍为「待确认」+ §4 作者删除行 Q3 残留待确认 → **PD 修正为「确认清单（全部已确认）」**
- **TASK_BOARD 冲突解决**：保留 PM v1.1 信息（PRD v1.1 / owner 决策全集 / 流程「开发中」）+ 我的「严格按设计文件开发」要求；rebase 完成
- **最终状态**：RQ-22-D ✅（owner 13:11 确认）→ RQ-22-FE/FS 🔴 P0 待认领（严格按设计文件）；广播区 PM 13:12（启动开发）+ PD 13:16（严格按设计）双广播

> — 产品设计师（PD） 2026-08-07 13:18

---

## [2026-08-07 13:16] 📋 PD 正式建单 · RQ-22-FE / RQ-22-FS · owner 确认设计 · 要求严格按设计文件开发 | 会话：[产品设计师(PD)]

owner 13:16 指令：「设计确认，接下来给相关的角色创建任务，无比把详细的设计文件同步给他们，并要求严格按照设计文件进行开发」。
- **TASK_BOARD 正式建单**：RQ-22-FE（前端开发）🔴 P0 · RQ-22-FS（全栈开发）🔴 P0，均标注「严格按设计文件开发」，依赖 RQ-22-D ✅
- **广播区新增 13:16 广播**：@FE @FS 立即认领 + 实现依据 4 份（交接单 / 设计文档 v1.0 / PRD v1.0 / demo URL）+ 红线
- **严格要求**：设计文档 v1.0 + 开发交接单为唯一实现依据，与设计不符先与 PD 对齐，不得自行偏离
- **设计文件已同步**（交接单内嵌全部参数）：交接单 `docs/04-开发实现/RQ-22-意见反馈留言板-开发交接单-20260807.md`（FE §三 rpx/交互/AC · FS §四 接口/字段/合规）+ 设计文档 + PRD + demo

> — 产品设计师（PD） 2026-08-07 13:16
...
- 产出：D-02 增量文档 + demo（按 owner 工作流：先 demo 确认 → 建 FE 任务）

> — 产品设计师（PD） 2026-08-07 12:23

---

## [2026-08-07 12:48] 🔄 PD 拉取 PM PRD v1.0 补充 · RQ-22-D 设计/demo 同步对齐 | 会话：[产品设计师(PD)]

owner 12:48 指令：「再去拉取一下 pm 关于需求细节的补充，来相应设计 ui」。
- **PM 补充已拉取**：commit `750853b`（12:39）· PRD v0.1 → **v1.0（已确认）**，新增 §4.3 作者识别与管理入口设计：
  - 作者识别：openid 白名单 → 环境变量 `AUTHOR_OPENID`（单作者 owner）· 云端校验不可伪造
  - 管理入口：页面内动态渲染（isAuthor 驱动）· **删除按钮卡片右下角** + **筛选 nav-bar 右侧下拉**（全部/仅违规标记/仅我的回复）
  - 作者昵称固定「**一页君**」；普通用户「微信用户 + 随机 4 位」
  - 软删除（status='deleted'）普通用户不可见、作者可查看（Q3 待确认）
- **PD 同步更新**：
  - `docs/02-产品设计/D-02-增量-RQ22-意见反馈留言板-UIUX设计.md` v0.1 → **v1.0**：删除位置改卡片右下角、筛选改 nav-bar 右侧下拉、作者昵称「一页君」、AC 10 → **12 条**（补 AC-11 个人主体 / AC-12 回归全绿）、FS 交接补 AUTHOR_OPENID 方案
  - `demo/feedback-demo.html` 同步（删除右下角 / 筛选下拉 / 一页君 / 维度 3 项）
  - TASK_BOARD RQ-22-D 行 + 说明更新
- **待 owner 确认**（PRD §八，不阻塞设计）：Q1 字数限制 · Q3 软删除作者可见 · Q4 是否展示「共 N 条」
...
**owner 12:14 反馈**：
1. 浏览历史 / 收藏页文字太贴屏幕边缘 → 优化（**020 已把左右留白加到 32rpx**；若真机仍贴边 → 加大至 40rpx）
2. 收藏页：分类下方提示词删掉（**020 已删**）；「共 x 条」从右侧移到**分类胶囊下方一行**（增量）
3. 长按取消弹窗 → **左滑滑出取消选项**（favorites **020 已实现左滑「取消收藏」**；history 无删除功能，是否加左滑删除待确认）

**任务**：BUG-20260807-003 已建 @FE（TASK_BOARD 3145 行）。前置 BUG-20260806-020（`5344f72`）。
...
**编排器**（RequestId `8d98ee1d`）：
- 日志：「异步触发 5 个分类…刷新已触发(编排/异步)」——**编排耗时 445ms**（此前同步等待 15.6s → 5 分片全 shard_error）
- 立即返回 `async=true`「刷新已触发，正在后台更新」，前端不再 15s 超时

**tech worker**（RequestId `7b47f580`）：
- 搜索阶段 **27.2s**：智谱 15.1s 超时（新 15s 生效）→ Qwen 12.1s 超时（新 12s 生效，config 遮蔽已修复）→ 30s 预算耗尽转聚合
- tianxing 兜底 5 条 → enrich **8.0s**（5/5 正文、5/5 AI 摘要）→ 备份 5 条
- 总耗时 **35.86s**（23:31 同分类 50.44s，**−14.6s**）

数据链路完整：worker 独立跑完写 `news_cache` + `backup`；编排器不再误报 0。

**说明**：worker 仍 ~36s 属环境客观耗时（AI 引擎集体故障 + juhe 每日配额耗尽），异步化已让用户免于干等；DG-09 短路需 1301 场景才会触发、DG-10 摘要长度需查 DB 复核，本轮日志未覆盖这两条路径。

> — 全栈开发（FS） 2026-08-06 23:54
...
- enrich 健康：5/5 正文、4-5/5 AI 摘要；距 55s 硬期限仅 ~1.3s 余量
- **关键**：部署版仍是 DG-09 之前的旧代码（无「确定性失败短路」日志）；且 DG-09 只治 1301 型失败，**治不了全引擎超时型**

**已实施**（`zhipuSearch.js`）：
| 常量 | 旧 | 新 | 依据 |
|------|-----|-----|------|
| SEARCH_PHASE_BUDGET_MS | 40s | **30s** | 全引擎故障串行耗满 40s，enrich 仅剩 ~13s |
| ZHIPU_TIMEOUT | 20s | **15s** | 健康 ~3s，仍留 5 倍余量 |
| QWEN_SEARCH_TIMEOUT | 15s | **12s** | 健康 ~10s |
| DEEPSEEK_SEARCH_TIMEOUT | 15s | **10s** | 当前 402/超时基本不可用 |

**预期**：全引擎故障时搜索 40s → 30s，enrich 窗口 ~13s → ~23s，总耗时 ~54s → ~46s（余量 14s）。

**待部署**：`refreshNews`（云端安装依赖，DG-09+DG-10+DG-11 一并生效）。

> — 全栈开发（FS） 2026-08-06 23:35
...
| 阶段 | 耗时 | 明细 |
|------|------|------|
| 搜索 | **40.16s** | 智谱 16.7s → 400 `code=1301` 内容安全拦截（**确定性**，连续两轮命中）→ Qwen 15.1s 超时（账户欠费）→ DeepSeek 8.4s 超时（40s 预算截断）|
| 聚合/天行 | 1.4s | juhe 10012 配额耗尽（0.27s）→ tianxing 130 限流重试后成功（5 条）|
| enrich | 12.27s | 5/5 正文覆盖（100%），3/5 AI 摘要；**距硬期限（catStart+55s）仅 1.15s** |
| 写库/清理/备份 | 0.6s | 写入 5 条 |
| **合计** | **54.16s** | P0-2 硬期限生效（enrich 没顶爆），但搜索吃掉 41s 后 enrich 窗口只剩 ~13s |

**根因**：搜索链串行 3 个 AI 引擎 + 各自 15~20s 超时，而 recommend 的 prompt 连续触发智谱 1301 内容安全拦截（确定性失败），却仍继续白耗 Qwen/DeepSeek 共 23.5s。

**已实施**：
1. **P0-3 · 确定性失败短路**（`zhipuSearch.js`）：新增 `isFatalSearchError`（匹配 1301 / 内容安全 / 账户欠费 / 封禁）。智谱或 Qwen 命中终态错误时**跳过后续 AI 引擎**，`return { news: [], engine: 'none' }` 直转聚合/天行。预期 recommend 刷新 54s → **~30s**，enrich 窗口 ~13s → ~36s。
2. **P0-4 · juhe null 崩溃修复**（`sources/juhe.js`）：10012 配额耗尽的 `resolve(null)` 漏到重试循环后，`rawList.length`/`rawList.map` 对 null 解引用抛 TypeError（日志 "Cannot read properties of null (reading 'length')"）——虽被外层 catch 兜住返回空，但错误信息误导排查。加 `(rawList || [])` 防护。

**待部署**：`refreshNews`（云端安装依赖）。
**待验证**：下次 recommend 刷新若智谱 1301，日志应出现「智谱确定性失败 → 跳过 Qwen/DeepSeek，直接聚合/天行兜底」且总耗时 ≤35s；juhe 配额耗尽时不再出现 null 崩溃报错。

> — 全栈开发（FS） 2026-08-06 23:12
...
**实现**（commit `4e92b90`）：
- owner 反馈：'今日新闻'下方新闻分类/新闻列表太靠上，上方页面有点挤
- 修复：`.panel-body` top 从 `menuTop+menuHeight`（胶囊底部）增加 **16rpx** 间距
- 让"今日新闻"标题与下方内容之间多 16rpx 呼吸空间（≈8px @ 375 屏）

**范围**：仅 `pages/home/home.wxss`，其他页面不动。

**交接**：**@owner** 真机体验侧边栏"今日新闻"与下方内容间距是否合适。

> — 小程序前端开发（FE） 2026-08-06 22:52
...
4. **P0-2 · refreshNews enrich 加硬期限**：`enrichNewsList` 新增 deadline（catStart+55s）——预算不足自动跳过 AI 摘要保正文、剩余 <3s 不再启动新条目，防 60s 超时 0 写入，提升 `news_cache.content` 覆盖率（提交 `415bccb`）。

**待部署**：`getNewsDetail` + `refreshNews`（云端安装依赖）+ `pages/detail/reading-engine.js` 随小程序发布。
**待验证**：部署后真机复测进入详情/翻页耗时；getNewsDetail 日志应不再出现「AI 摘要生成成功」行；refreshNews 日志应看到 enrich 预算保护（预算紧张跳摘要、有 content 条目占比提升）。
...
1. 「推荐 · N 条」副标题从「今日新闻」右侧 → **移到下方**（header 改 column 左对齐）
2. 左侧分类区（panel-wheel）**整个区域上下滑动**都能切分类（触摸热区从 128+32rpx 扩展到 ~240rpx）

**范围**：仅侧边面板微调；首页 + 其他不动；不涉及 D-10 重新设计文档（已暂缓）。

**任务**：BUG-20260806-010 已建 @FE（详见 TASK_BOARD 任务表 2868 行），AC：
- 副标题在「今日新闻」下方左对齐
- wheel 触摸热区 ~240rpx 整区响应滑动切分类
- panel-body top 同步上调（容纳新 header 高度）
- 视觉 wheel 仍 128rpx；首页无回归；真机通过

> — 产品设计师（PD） 2026-08-06 18:34

## [2026-08-06 18:28] ⏸ D-10 侧边新闻列表页重新设计 · 暂缓 · 不建 FE 任务 | 会话：[产品设计师(PD)]

owner 18:28 决定「暂时不讨论这个方案」：
- ❌ 未建 FE 任务；D-10 方案取消推进（文档/demo 保留仓库，状态改「已暂缓」）
- ✅ 首页 UI/UX 保持现状不变
- TASK_BOARD 广播区已登记暂缓公告；D-10 文档状态已改「已暂缓」

> — 产品设计师（PD） 2026-08-06 18:28

## [2026-08-06 17:47] ✅ owner 真机验收 · 全部待验项通过 · 001/006 + 004/005/005追修/007/008/009 + AB-01 + DG-01~05 正式关闭 | 会话：[产品设计师(PD)]

**owner 17:47 真机反馈**：「BUG-20260802-001 以及 BUG-20260802-006 以及全都通过」→ 所有待验收项真机通过，正式关闭：

- ✅ **BUG-20260802-001** 详情页翻页单向失效 → 关闭（FS 修复 + owner 真机通过 17:47）
- ✅ **BUG-20260802-006** 切分类 ~0.5s 提示 → 关闭（FS 修复 + owner 真机通过 17:47）
- ✅ **BUG-20260806-009** 状态栏不透明 + 按钮胶囊对齐 → 关闭（v3 背景层方案，owner 真机通过 17:47）
- ✅ **BUG-20260806-008** 侧边栏滚轮三优化 + 分类上移 → 关闭（面板滑动 v3，owner 真机通过 17:47）
- ✅ **BUG-20260806-007 / UAT-03** D-09 v1.2 系统级导航 → 关闭（owner 真机通过 17:47）
- ✅ **BUG-20260806-004 / 005 / 005追修** 收藏静默化 / 滚轮淡蓝 / 去背景高亮 → 关闭（owner 真机通过 17:47）
- ✅ **AB-01** 关于一页 → 关闭（owner 真机通过 17:47）
- ✅ **DG-01~05** 数据治理 → owner 真机通过 17:47（含云函数部署后验证：首页推荐 / 首屏 10 条 / 翻底 5×3 / 收藏纯本地 30 天）

**剩余**：DG-06 全量回归待 PM 排期（不属于 owner 真机验收范围）。

台账同步：TASK_BOARD 10 行状态 + 顶部广播区新增 17:47 汇总；Bug 清单模板同步关闭。

> — 产品设计师（PD） 2026-08-06 17:47
...
**根因**：`.panel-wheel { align-items: center }` 垂直居中——wheel 高 432rpx（6 行 × 72rpx），面板高约 1100rpx，上方留约 300+rpx 空白，远超「1 个分类名空间」。

**修复（PD 落地）**：
- `pages/home/home.wxss` `.panel-wheel`：`align-items: center → flex-start`（顶部对齐）+ `padding: 20rpx 0 → 8rpx 0`
- 效果：wheel 顶部对齐面板，选中固定位（第 2 行）上方仅剩第 1 行一个分类名空间
- 不变量：选中锚点（第 2 行）/ 项高 72rpx / 可视 6 行 / 触摸热区外扩 / 连续跟手 snap 均不变

**同步更新**：
- D-02 文档追加 v2.7 变更说明（版本号 + 布局规则）
- Bug 台账 008：状态 → 🔄 待验证（v2.7 已修，待 owner 真机复验），追加 AC-v2.7
- TASK_BOARD 008 行状态同步

> — 产品设计师（PD） 2026-08-06 16:15

## [2026-08-06 16:05] 📤→✅ FS 已提交 · commit `b2114dd`（recommend 抓取量 10→8 条/轮 · DG-01 超时实证调整）| 会话：[全栈开发(FS)]

**提交记录**：commit `b2114dd`（zhipuSearch.js RECOMMEND_COUNT 10→8）

**触发**：16:01 云端日志实证 recommend 10 条 prompt 触发智谱 50s 超时（ZHIPU_TIMEOUT），总耗时 56.8s 逼近 60s 云函数上限（余量 3.2s）→ FS DG-01 超时预警被实证。

**调整**（owner 16:05 确认）：RECOMMEND_COUNT 10→8 条/轮（智谱 ~45s 内完成，降级链余量充足）。

**🔔 联动**：PM 确认 R5 首屏 10 条口径（DG-03 用「8+翻底拉取」适配）；owner 重新部署 refreshNews 复验；DeepSeek 402 余额不足提醒（同日 2 次）。
...
## [2026-08-06 15:15] 📋 PD 处理 · 008 滚轮交互三优化提单（owner 15:12）+ D-08 深色复验通过闭环 | 会话：[产品设计师(PD)]

**提交记录**：
- commit（PD 前缀）：D-02 v2.6 + Bug 008 提单 + D-08 闭环 + TASK_BOARD 广播
- 文件：`docs/02-产品设计/D-02-增量-侧边分类滚轮.md`（v2.6）、`docs/05-测试验收/Bug清单模板.md`（008 详述）、`docs/02-产品设计/D-08-设计规范一致性自查-20260806.md`（owner 复验闭环）、`TASK_BOARD.md`

**owner 15:12 反馈**：
1. **005 追修没问题**（去底色 ✅）；
2. **D-08 深色复验通过** → D-08 闭环（G-07 残留 51→0，豁免 9 + 深色专用覆盖 4）；
3. **DG-01~05 真机验证：待验证**；
4. **滚轮 3 点提升** → 提 BUG-20260806-008（P1）：
   - ① 选中锚点修正：选中项恒第 2 行、顶部恒留 1 分类名空间（首类留空行）——根因 `Math.max(0,·)` 使 idx≤1 贴顶，改 `ty=(anchorIndex-idx)*itemHeight`；
   - ② 触摸热区：整条 128rpx×6 行 + 外扩可触摸，不限于文字命中；
   - ③ 滚动连续性：拖动连续跟手（非按格跳变），松手 300ms snap，bounce 保留；
   - D-02 升 v2.6（AC-RQ15-18/19/20 新增）；修复建议 + 验收标准见台账 008。

**当前在途**：007（D-09 系统级落地，FE 已交付 7693b0d，待 PD 验收）/ 008（滚轮三优化，FE 待处理）/ DG-01~05 真机待 owner 验证 / DG-06 全量回归待 PM 排期。
...
## [2026-08-06 13:22] ✅ owner 真机验收通过 · BUG-20260806-006 收藏/浏览历史同步问题正式关闭 | 会话：[小程序前端开发(FE)]

**提交记录**：TASK_BOARD 13:22 关闭广播 + 任务表 BUG-006 → ✅ 关闭 + Bug 台账状态同步

**owner 反馈**：「上面关于收藏、浏览历史的问题验收通过了，可以关掉了」

**BUG-20260806-006 闭环链路**：
- FE 修复 `5b87b1d`（方案 1 全局单例：detail/favorites/history 统一 `localCache`，容量 200→500）
- PD 代码级验收 ✅（13:20，含回归 b06 20/0 / v13）
- **owner 真机验收通过（13:22）**：收藏→列表立即出现 / 取消→立即消失 / 浏览历史同源生效

**当前闭环状态**：BUG-001/002/005追修/006 + UAT-01/02/04/05 全部通过；BUG-004（导航胶囊对齐）PD 代码级通过、真机覆盖项并入 UAT-03（标题顶置）待 D-09/BUG-007 落地后回归。
...
**提交记录**：
- commit（FE 前缀）：移除首页 `top-bar-settings` 按钮 + 样式清理
- 文件：`pages/home/home.wxml`、`pages/home/home.wxss`、`TASK_BOARD.md`（13:22 广播）

**背景**：该按钮为 BUG-20260806-004 导航对齐时误加——wxml 绑定 `onSettings` 但 home.js 无此方法（死按钮），且与 ⚙ dock 菜单设置入口重复。
**处理**：移除按钮与相关样式；`fixed-top-bar` 保留空占位（结构/胶囊对齐不变）；`settings.svg` 图标保留（dock 菜单使用）。
**设置入口现状**：仅 ⚙ 浮动「更多功能」菜单 →「设置」。

---

## [2026-08-06 13:20] 📋→✅ PD 验收 · BUG-20260806-006 / 005追修 / D-08 方案A 代码级全部通过 | 会话：[产品设计师(PD)]

**提交记录**：
- commit（PD 前缀）：台账验收记录 + 005/006 状态更新 + TASK_BOARD 13:20 广播
- 文件：`docs/05-测试验收/Bug清单模板.md`、`TASK_BOARD.md`

**① BUG-20260806-006 收藏列表不显示（FE `5b87b1d` 方案 1 全局单例）→ ✅ 通过**
- detail/favorites/history 统一全局单例 `localCache`（容量 200→500）；全项目业务代码 `new LocalCache` 零残留
- 回归：b06 20/0 ✅ / v13 ✅ → **@owner 真机确认**「收藏→列表立即出现」

**② BUG-20260806-005 追修（FE `9bb5b60` 去背景高亮）→ ✅ 通过**
- `.wheel-label.active` 移除 background-color/border-radius，仅淡蓝字体；transition 清理；`--wheel-bg-selected` 保留（v13 契约）
- js/wxml 零改动 → **@owner 真机确认**去底色视觉效果

**③ D-08 方案 A（FS `639746b` 51 处兜底删除）→ ✅ 通过**
- 8 文件 +35/-213；深色专用覆盖保留；`--color-warning-subtle` 双变量块值一致 + 2 处琥珀改 var()
- v13 88/0 ✅ / v11 25/0 ✅ / v7 63/0 ✅ → **@owner 深色复验**（浮层/按压/整体）

**⚠️ 待同步**：D-02 v2.4「淡蓝底色保留」→ 改「去底色」（005 追修后规范变更），PD 跟进更新。
...
**闭环确认**：AI 摘要「首页未显示」问题从 08:05 排查至 12:59 完整闭环——云端部署（owner）+ 环境变量（owner）+ getNewsList v7.1（owner 部署）+ 前端 wxml 遮蔽修复（FS `9a5b0db`）→ 真机可见 ✅

**TASK_BOARD**：11:15 FS 闭环广播已标注 ✅ owner 真机确认（12:59）。

**🔔 后续**：TL-B18 验收补充运行时渲染断言（PD）；BUG-006 已由 FE `9bb5b60` 全局单例方案修复（见下方 FE 交付）；FS 待办 D-08（兜底块评估）+ 数据治理任务分工拍板（PM `b643dbf` 提请）。
...
**提交记录**：
- commit（PD 前缀）：D-09 v1.1 系统级规范 + 展示页 v1.1 + 画廊 desc 同步
- 文件：`docs/02-产品设计/D-09-增量-导航栏内容起始位置规范.md`（v1.0 → v1.1）、`docs/showcase/d09-nav-content-spacing.html`（v1.1）、`docs/index.html`（画廊 desc）
...
**D-09 v1.1 已更新**：
1. **导航条带与胶囊呼吸空间**（§3.1）：条带 `top = menuTop - 12rpx`，`height = menuHeight + 24rpx`（上下各 12rpx 6px 呼吸）；按钮 top:50% translateY(-50%) 垂直居中 → **条带对称扩展 → 条带中心 = 胶囊中心 → 按钮中心 = 胶囊中心**（锚定不变）；
2. **进度条同色化**（§3.2）：去掉底槽（与页面同色），仅保留 3rpx 浅蓝进度线；位于条带底部（胶囊下缘下 6px）；
3. **系统级落地**（§四）：6 页面统一（detail / favorites / history / settings / about / home），home 同步导航条带呼吸规范（设置按钮锚定胶囊中心）+ 卡片居中不变；
4. **AC v1.1**（§五）：8 项验收标准，含胶囊呼吸 / 按钮锚定 / 进度条同色等；
5. **专属展示页 v1.1**（§六）：对比 3 处差异（标题位置 + 胶囊呼吸 + 进度条同色），已用 Chromium 截图验证渲染正常。
...
**等 owner 确认 5 项**（§七）：① 导航条带胶囊呼吸 12rpx（6px）？② detail 标题呼吸 32rpx（16px）？③ 列表页 16rpx（8px）？④ 进度条无底槽？⑤ 正文无分割线保留？
...
**提交记录**：
- commit（PD 前缀）：D-08 自查报告 + TASK_BOARD 广播
- 文件：`docs/02-产品设计/D-08-设计规范一致性自查-20260806.md`（新增）、`TASK_BOARD.md`（10:20 广播）

**结果**（全仓 9 个 wxss 裸色扫描）：
- ✅ 达标：about / category-wheel / share-card 0 裸色
- ⚠️ **核心发现：51 处「暗色兜底硬编码块」**（home 20 / favorites 12 / settings 8 / history 7 / detail 4）——BUG-20260805-002 兜底，D-07 S2 仅迁移选择器未回归 var()，与 theme.json 双轨并行 → 漂移风险，**G-07 治理残留升级**
- 🟡 语义色缺 token ×2（琥珀背景）→ 建议 `--color-warning-subtle`
- 🟢 已知残留维持（浅色按压态 P3）；font-panel 特例可选优化
- 豁免 9 处（阴影/遮罩/iOS 白）

**建议**：方案 A（评估后删除兜底块回归 var()）/ 方案 B（兜底纳入 v13 契约测试）→ @FS/FE 工程判断执行，PD 验收。
...
## [2026-08-06 10:10] 📤→✅ FS 已提交 · commit `57b35cf`（数据治理 §五 5 项技术评估 + 2 个需求遗漏点补充）| 会话：[全栈开发(FS)]

**提交记录**：
- commit `57b35cf`：`docs/04-开发实现/数据治理-5项FS评估-20260806.md` + `TASK_BOARD.md`（10:10 广播）
- 前置补记：commit `d52dd56`（COMMLOG 补记 e910b7a 交付）

**5 项评估结论**（应 PM 09:48 要求，评估 §五 待确认项）：
| # | 结论 |
|---|------|
| ① | 推荐分类复用现有 recommend 数据（零新增数据源）；⚠️ 数据量缺口：5 条/轮 vs 首屏 10 条 → 建议抓取量提至 10 |
| ② | 云端历史/收藏数据直接废弃 + 广播标注，不做迁移 |
| ③ | 本地上限采纳历史 500 / 收藏 200，常量化 |
| ④ | 跨分类跳转固定顺序（推荐→科技→国际→科学探索→社会），删除后台补拉根治 P1/P2 |
| ⑤ | 每日清理异步延迟 2s，惰性过滤为主防线 |

**⚠️ 需求文档遗漏 2 点（P0）**：detail.js `_recordBrowse` 云端 report、`_syncFavoriteCloud` 双写——云函数停用后会失败入队重试，须纳入开发范围（追加 3 文件）。

**🔔 下一步**：PM 创建开发任务派发（预估 2-3 单元）。
...
**提交记录**：
- commit（PD 前缀）：TASK_BOARD 任务表新增「📋 当前焦点 · 真机验证任务」区块（UAT-20260806-01~05）+ 广播
- 文件：`TASK_BOARD.md`（任务表 + 09:55 广播）

**任务清单**（owner 认领，验证后回填状态 → PD 关闭/重开）：
| ID | 任务 | 优先级 |
|---|---|:---:|
| UAT-20260806-01 | 001 收藏/取消收藏 toast 验证 | 🟢 |
| UAT-20260806-02 | 002 收藏/历史跳转精准验证（重点）| 🔴 |
| UAT-20260806-03 | 003 导航按钮层级验证 | 🔴 |
| UAT-20260806-04 | D-07 真机三态验证（G-01/G-03 标志用例）| 🔴 |
| UAT-20260806-05 | 深色模式 icon 视觉确认（dock/导航/空态）| 🟡 |

> 验证要点/通过标准已写入任务表每行；owner 验证后同步结果。
...
**提交记录**：
- commit（PD 前缀）：004 正式提单 + D-02 侧边分类滚轮规范更新 v2.4 + TASK_BOARD 广播
- 文件：`docs/05-测试验收/Bug清单模板.md`（004 详述）、`docs/02-产品设计/D-02-增量-侧边分类滚轮.md`（v2.4）、`TASK_BOARD.md`（08:02 广播）

**流程**：owner 要求"提之前再次把设计发给我们确认" → PD 先出 13 项 review 差异表（附设计出处 D-02 v2.2/v2.3）→ owner 08:02 确认并裁定 → 正式提单 ✅

**BUG-20260806-004 · 侧边栏分类滚轮选中态字体淡蓝（P1）**：
- owner 裁定：**仅改选中态字体为淡蓝 `--primary`**（浅 `#007AFF` / 暗 `#0A84FF`），淡蓝底色保留；panel-wheel / 超界隐藏 / 项数 5 项 / 触摸手势 / active 触摸态 / 选中锚点 **全部保持现状**
- 根因：`category-wheel.wxss` `.wheel-label.active` 文字色沿用 `--wheel-text-selected`（rgba .5 深色）
- 修复红线：仅改选中项 color → `var(--primary)`；不动宽度/分隔线/项数/手势/锚点/可视 6 项/震动/bounce
- 设计同步：D-02 更新至 **v2.4**（§2.1 选中文字 + §5 Token 表 + AC-RQ15-10 标注"本版本不执行"——侧边栏保留分类滚轮）

**review 中已澄清、非本次 bug 范围**（owner 裁定以当前为准）：组件位置在侧边栏内（非右侧常驻）、宽度 128rpx、panel-wheel border-right 分隔线、5 项分类与命名、"全部"首项锚点贴顶——均保持现状。

---

## [2026-08-06 07:40] 📤→🔴 PD 已提单 · owner 直报 3 项缺陷（BUG-20260806-001~003）| 会话：[产品设计师(PD)]

**提交记录**：
- commit（PD 前缀）：代 owner 录入 3 项缺陷提单 + 代码级根因定位 + TASK_BOARD 广播
- 文件：`docs/05-测试验收/Bug清单模板.md`（3 项详述）、`TASK_BOARD.md`（07:40 广播）

**三项缺陷（owner 真机直报）**：
1. **BUG-20260806-001 · 收藏/取消收藏无反馈提示（P2）**：`detail.js` `onToggleFavorite` 两分支均缺 toast → 补「已收藏/已取消收藏」；后端 `setUserFavorite` 语义完备无需改接口（可选：云端补 200 上限校验，P3）
2. **BUG-20260806-002 · 收藏/历史点击跳转非选中新闻（P1）**：`reading-engine.js` `_buildMergedList` 按 newsId 未命中时**静默回退分类首条**（入口 index 缺省 0）→ 旧新闻必现跳错。数据已真实落库 ✅（favorites / browse_history，历史 7 天 TTL），缺陷纯前端跳转链路。推荐方案 A：详情页单篇模式；失效 id 明确提示不展示他条
3. **BUG-20260806-003 · 详情页顶部按钮为主页非返回（P1）**：`detail.wxml` 顶部 `nav-home`→`goHome`（reLaunch 清栈），`goBack` 已实现未绑定。规则：第 3 层及以上主页按钮→返回按钮；第 2 层（收藏/历史）保持主页按钮 ✅；settings/about 是否统一待 owner 裁定

**🔔 指派**：FE 主修 3 项，FS 确认 001/002 后端，PM 建任务行排期；修复后 PD 按台账验收标准逐项代码级验收。
...
**提交记录**：
- commit（PD 前缀）：D-07.1 追加 §十一 综合验收 + TASK_BOARD 广播
- 文件：`docs/02-产品设计/D-07-走查底稿确认与现状盘点.md`（§十一）、`TASK_BOARD.md`（广播）

**验收结论（4 项交付）**：
1. **`70e5d02` D-07 S1-S4 · 条件通过**：G-01~G-05/G-08 代码级确认关闭（状态机始终注入/onThemeChange/窗口同步/组件 isolated/effectiveTheme）；G-07 收敛至 10 处裸露（8 豁免），**残留 2 处**（favorites:195、history:146 按压态）；.page--light 缺 2 变量（P2）
2. **`da0cae6` AB-01 · 通过**：PRD 6 项验收标准 + 文案逐字一致；dock 排列正确；wxss 全 token
3. **`e29290f` 滚动修复 · 通过**：4 页 min-height:0+overflow-y:auto
4. **`88662ec` 编译彻底修复 · 通过**：注释内 / 全清，变量块完整，v13 84/0

**📋 4 项跟进（不阻断）**：
① v7 L182 断言过期（boundary-chip 已清理但断言仍在）→ **PM 同步测试**（v7 现 62/1）
② G-07 残留 2 处按压态 → FS 补 .page--dark 覆盖
③ .page--light 缺 --color-favorite-active/--setting-panel-blur → FS 补
④ 图标规范漂移（文档描边式 vs 实际填充式，历史遗留）→ PM/PJM 决策

**🔔 owner**：真机三态验证（系统深色+手动浅色=G-01 标志用例；手动深色+系统浅色轮盘=G-03 标志用例）→ 通过后 D-07 正式闭环
...
**提交记录**：
- commit（PD 前缀）：D-07.1 追加 §十 编译错误验收记录 + TASK_BOARD 广播
- 文件：`docs/02-产品设计/D-07-走查底稿确认与现状盘点.md`（§十）、`TASK_BOARD.md`（广播）

**验收结论：✅ 通过**（FE `f26b379` app.wxss 编译错误修复）
1. **根因确认**：f3bf107 注释内 `--flash-*/--wheel-*/` 含 `*/` → WXSS 提前终止注释 → `Unexpected '/'`
2. **修复彻底**：全仓 9 个 wxss 逐字符扫描无孤立 `*/`；仅注释文本改动，变量块未动（无视觉漂移）
3. **回归**：v13 84/0 + 全量套件全通过

**PD 自省**：f3bf107 验收遗漏「可编译性」检查——验收清单补「改动文件编译级冒烟」，纳入 D-07.1 §七 走查计划

**防复发（提请）**：① PJM/PM 落红线「WXSS 注释内禁止 */ 序列」；② PM v13 追加注释 `*/` 断言

---

## [2026-08-05 18:40] 📤→✅ FE 已提交 · commit `f26b379`（紧急修复：app.wxss 编译错误 `Unexpected '/'`）| 会话：[小程序前端开发(FE)]

**提交记录**：
- commit `f26b379`：app.wxss 第 16 行注释内 `--flash-*/--wheel-*/` 含 `*/` 序列，微信 WXSS 编译器将其误判为注释结束符，导致后续 `/--wheel-*/...` 被当作代码触发 `Unexpected '/'`。将 `*/` 改为 `★` 消除歧义。

**根因分析**：CSS 标准中 `*/` 是注释结束标记。虽然标准要求完整 `/* ... */` 配对，但微信开发者工具 WXSS 编译器使用简单字符串扫描，遇到注释内的 `*/` 即提前终止注释。第 16 行注释文本中的 `--flash-*/--wheel-*/` 恰好触发此行为。
